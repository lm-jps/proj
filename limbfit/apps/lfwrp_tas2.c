...
//  {ARG_INT, 	"smpl", "0", "sampling factor: default=no sampling, otherwise: number/unit eg: 4: 1/4 of the program (1 seq, skip next 3 seqs). this is combined with cam and fid"},
  ...
  {ARG_INT, 	"src", "0", "0: hmi.lev1 (default), 1: hmi.lev1_nrt  (quality test is different) 2: limbfit_annulus (initial test is different)"},
...
int DoIt(void)
{
	...
	if(src == 0) 
		...
		if(src == 1) 
			...
			{
				fprintf(stderr, "src to 0, 1, or 2\n");
				return(ERR_EXIT);
		    }
