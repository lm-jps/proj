...
int get_set_kw(int typ, char *kw, char *kw_txt, unsigned int fsn, DRMS_Record_t *record_in,DRMS_Record_t *record_out, fitsfile *outfptr, FILE *opf, int debug, int *status)
{
	...
	if ( rstatus != 0)
	{
		...
		write_mini_output(PROCSTAT_NO_LF_FITS_WRITE_PB,record_in,record_out,opf,debug); 
		*status=ERR_DRMS_READ_MISSING_DATA;   
		return ERR_FITSIO;
	}
return(0);
...
// need to test status...
			...
			rstatus=drms_setkey_int	 (record_out, "NUM_EXT",	num_ext);  
			...
			data_array2 = drms_array_create (DRMS_TYPE_FLOAT, 2, i_naxes, (void *)lfr->fits_fulldfs, &rstatus) ;
			data_array2->bscale = 1.0;
			data_array2->bzero = 0.0;
			rstatus = drms_segment_write (segment_out, data_array2, 1);
			if (rstatus) 
			{
			...
			data_array3 = drms_array_create (DRMS_TYPE_FLOAT, 2 ,i_naxes, (void *)lfr->fits_alpha_beta1, &rstatus);
			data_array3->bscale = 1.0;
			data_array3->bzero = 0.0;
			rstatus = drms_segment_write (segment_out, data_array3, 2);
			if (rstatus) 
			{
			...
			data_array4 = drms_array_create (DRMS_TYPE_DOUBLE, 2, i_naxes, (void *)lfr->fits_params1, &rstatus) ;
			data_array4->bscale = 1.0;
			data_array4->bzero = 0.0;
			rstatus = drms_segment_write (segment_out, data_array4, 3);
			if (rstatus) 
			{
...
// something left...: in case of problem, filenameout not closed and therefore not removed...
			...
			if ( fits_update_key(outfptr, TINT, "NUM_EXT", &num_ext, "Number of extensions",&rstatus) )
			{
				lf_logmsg4fitsio(log_msg, log_msg_code, "(NUM_EXT)",fsn,rstatus,opf);			
				write_mini_output(PROCSTAT_NO_LF_FITS_WRITE_PB,record_in,record_out,opf,debug); 
				return ERR_FITSIO;
			}
			...
			for (i=1;i<=lfr->fits_ab_tfields;i++)
			{
				if (fits_write_col(outfptr, TFLOAT, i, firstrow, firstelem, lfr->fits_ab_nrows, 
												lfr->fits_alpha_beta2+(i-1)*lfr->fits_ab_nrows, &rstatus))
				{
			...
			for (i=1;i<=lfr->fits_params_tfields;i++)
			{
				if (fits_write_col(outfptr, TDOUBLE, i, firstrow, firstelem, lfr->fits_params_nrows, 
												lfr->fits_params2+(i-1)*lfr->fits_params_nrows, &rstatus))
				{
		...
		else 
		{
			...
			write_mini_output(PROCSTAT_NO_LF_FAILED,record_in,record_out,opf,debug); 
