...
#define KEYSTUFF_VERSION_NUM     (1.1)
...
int drms_verify_links (char *oldser, char *newser, int *keyct, int *segct) {
/*
 *  Check that any keywords or segments in newser that are defined as links
 *    into oldser actually do have the appropriate targets in oldser
 */
  DRMS_Record_t *nrec, *orec;
  DRMS_Keyword_t *nkey, *okey;
  DRMS_Segment_t *nseg, *oseg;
  DRMS_Link_t *nlnk;
  HIterator_t *last = NULL;
  int n, lnkct, status;
  char *linknam;
  int keylnkct = 0, seglnkct = 0;
					    /*  examine new series structure  */
  nrec = drms_template_record (drms_env, newser, &status);
  if (status) {
    fprintf (stderr,
  ...
  orec = drms_template_record (drms_env, oldser, &status);
  if (status) {
    fprintf (stderr,
	"Error: drms_template_record returned %d for data series:\n", status);
    fprintf (stderr, "       %s\n", oldser);
  ...
  while ((nlnk = drms_record_nextlink (nrec, &last))) {
    if (strcasecmp (nlnk->info->target_series, orec->seriesinfo->seriesname)) {
      lnkct--;
      continue;
    }
