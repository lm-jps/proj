      subroutine enudge_gl_ll(m,n,rsun,brt,elon,elat)
c
c+
...
c
      ...
      integer :: imh,jph,jmh
      real*8 :: f(m,n), et(m,n+1), ep(m+1,n)
...
c
      ...
      np1=n+1
c
c - - Now need to transpose and flip brt into the array f:
c - - Inside the subroutine we'll be working in spherical polar coordinates
c - - (co-latitude and azimuth), and then convert to lon, lat at the end.
c - - loop for transpose and flip:  Remember factor of rsun**2 for source term
c
      do i=1,m
         do j=1,n
            f(i,j)=-brt(j,mp1-i)*rsun2
         enddo
      enddo
c
...
c
      if(ierror .ne. 0) then
         write(6,*) 'enudge_gl_ll: error in hstssp, ierror = ',ierror
         stop
      endif
c
...
c
     ...
     1      (rsun*dtheta)
      ...
      enddo
c
c - - transpose, flip et, ep arrays (and for et, negativize) to get elon,elat
c
      do iph=1,m
         do j=1,n+1
            elat(j,m+1-iph)=-et(iph,j)
         end do
      end do
c
      do jph=1,n
         do i=1,m+1
            elon(jph,m+2-i)=ep(i,jph)
         end do
      end do
c
