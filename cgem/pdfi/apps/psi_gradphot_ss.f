      subroutine psi_gradphot_ss(m,n,p,bcn,a,b,c,d,rsun,psi3d,
     1           btphot,bpphot,divbh)
c
...
c
c   - Usage:  call psi_gradphot_ss(m,n,p,bcn,a,b,c,d,rsun,psi3d,
c             btphot,bpphot,divbh)
c
...
c
c - - Output: btphot(m+1,n) - real*8 array of potential field
c             B_theta at photosphere, TE grid [G]
c
c - - Output: bpphot(m,n+1) - real*8 array of B_phi component at photosphere,
c             PE grid [G] 
c
c - - Output: divbh(m,n) - real*8 array of div_h B_h at photosphere
c             on CE grid locations [G]
c
c-
...
c - - output variables:
c
      real*8 :: btphot(m+1,n),bpphot(m,n+1),divbh(m,n)
c
c - - Internal variables and arrays:
c
      integer :: q
      real*8 :: dtheta,dphi,pi,dum
...
c
      if((bcn .ne. 0) .and. (bcn .ne. 3)) then
         write(6,*) 'psi_gradphot_ss: Illegal bcn = ',bcn, ' exiting'
         stop
      endif
c
      pi=pimach(dum)
...
c     (commented out for this calculation since it contributes nothing)
c
c     area=(d-c)*(cos(a)-cos(b))*rsun**2
c
c - - get value of b0 by dividing mflux by area:
c
c     b0=mflux/area
c
c - - radial spacing (not needed here so commented out):
c
c     delr=rssmrs/p
c
...
c
      call gradh_ce_ss(m,n,psi2d1,rsun,sinth_hlf,dtheta,
     1        dphi,gradt,gradp)
      btphot(1:m+1,1:n)=-gradt(1:m+1,1:n)
      bpphot(1:m,1:n+1)=-gradp(1:m,1:n+1)
c
      call divh_ce_ss(m,n,btphot,bpphot,rsun,sinth,sinth_hlf,dtheta,
     1     dphi,divbh)
c
c - - we're done
c
