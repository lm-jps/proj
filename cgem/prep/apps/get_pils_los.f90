subroutine get_pils_los(naxis1, naxis2, Bmap, pilmap, thresh, dilation_param)

!
! PURPOSE: Given input "Bmap" array (Nx,Ny) corresponding to magnetogram 
...
!
  ...
  real*8, intent(in) :: bmap(naxis1,naxis2) ! input magnetogram, B_los or B_r
  ...
  real*8 thresh ! threshold in abs(Bmap) below which pixel values are ignored
...
!  naxis2 = size(bmap, 2)
  ...
  !print *,sum(posmap),sum(dilpos) ! diagnostics of dilation subroutine
  !print *,dilpos ! diagnostics of dilation subroutine
