...
int Dopplergram_largercrop(DRMS_Array_t **arrLev1p,DRMS_Array_t **arrLev15,int framelistSize,DRMS_Array_t *Lookuptable,float Rsun,float X0,float Y0,struct parameterDoppler DopplerParameters,int MISSVALS[5],int *SATVALS,float cdelt1,TIME TargetTime)
{
  ...
  int QUICKLOOK;
  float ExtraCrop = 90.;//we crop the data at Rsun+ExtraCrop in pixels
  ...
  double period = (double)(N-1)*dtune; //!!! WARNING, THIS IS WRONG: SHOULD BE (double)N*tune
  double pv1,pv2;
  ...
  pv1 = dvtune*(double)(N-1); //!!! WARNING, THIS IS WRONG: SHOULD BE (double)N*tune
  pv2 = pv1/2.;
