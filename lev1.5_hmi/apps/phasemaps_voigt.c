...
/* 10 new detune keywords added when this program was checked into CVS. 
 * -Art, October 20, 2015 
 */
...
static int setKey(DRMS_Record_t *rec, const char *key, DRMS_Value_t *value)
{
    int rv = 0;
    ...
    {
        rv = (drms_setkey_p(rec, key, value) != DRMS_SUCCESS);
    }

    if (rv)
    {
        fprintf(stderr, "Failed to set keyword %s.\n", key);
    }
...
#pragma omp for
      ...
      if (status == DRMS_SUCCESS)
	{
	  ...
	  //WRITE KEYWORDS
        ...
        val.value.float_val = (float)WBphase;
        if (setKey(recout, KEY_PHASE_WBMICHELSON, &val))
        {
            return EXIT_FAILURE;
        }

        val.value.float_val = (float)E1phase;
        if (setKey(recout, KEY_PHASE_LYOT, &val))
        {
            return EXIT_FAILURE;
        }

        val.value.float_val = (float)meanwidthg;
        if (setKey(recout, KEY_PHASE_LINEWIDTH, &val))
        {
            return EXIT_FAILURE;
        }

        val.value.float_val = (float)meandepthg;
        if (setKey(recout, KEY_PHASE_LINEDEPTH, &val))
        {
            return EXIT_FAILURE;
        }

        val.value.float_val = (float)meanIcg;
        if (setKey(recout, KEY_PHASE_LINECONTINUUM, &val))
        {
            return EXIT_FAILURE;
        }
