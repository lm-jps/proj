...
For IQUV observables obtained from definitive level 1 records and from a standard observable sequence with a 135s cadence, the output DRMS series is hmi_test.S_720s.
For IQUV observables obtained from quicklook/nrt level 1 records, the series is hmi_test.S_720s_nrt.
...
For IQUV observables obtained from definitive level 1 records and from a standard observable sequence with a 135s cadence, the output DRMS series is hmi_test.S_5760s.
...
\li \c lev1="series" where series is a string and is the name of the DRMS series holding the level 1 records to be used by the observables code. For normal observables processing either of these two series should be used: hmi.lev1_dcon_nrt for the quicklook/nrt level 1 records, and hmi.lev1_dcon for the definitive level 1 records. Or with hmi_test.lev1_dcon or hmi_test.lev1_decon_nrt.
The value by default is hmi_test.lev1_dcon (to be consistent with the default value of quicklook=0).
...
ModuleArgs_t module_args[] =        
{
     ...
     {ARG_STRING, SeriesIn, "hmi_test.lev1_dcon",  "Name of the lev1 series"},
...
{
  return strdup("$Id: HMI_IQUV_averaging_dcon.c,v 1.3 2016/10/03 17:43:52 arta Exp $");
}
...
#define MaxNString 512                                                //maximum length of strings in character number
  ...
  //*************************************************************************************
   ...
   if(QuickLook == 1)                                                //Quick-look data
     { 
       if(AverageTime == 720.0 && (DataCadence == 90.0 || DataCadence == 135.0)) strcpy(HMISeriesLev1p,"hmi_test.S_720s_dcon_nrt");
       else
	 { 
	   if(AverageTime == 720.0 && (DataCadence == 120.0 || DataCadence == 150.0)) strcpy(HMISeriesLev1p,"hmi_test.S2_720s_dcon_nrt");
	   else
	     {
       ...
       //if(AverageTime == 360.0) strcpy(HMISeriesLev1p,"hmi_test.S_360s_dcon_nrt");
       //if(AverageTime == 5760.0)strcpy(HMISeriesLev1p,"hmi_test.S_5760s_dcon_nrt");
     }
   else                                                               //Definitive Data
     {
       if(AverageTime == 720.0 && DataCadence == 90.0)  strcpy(HMISeriesLev1p,"hmi_test.S_720s_dcon"); //6 wavelengths (mod A)
       if(AverageTime == 720.0 && DataCadence == 135.0) strcpy(HMISeriesLev1p,"hmi_test.S_720s_dcon"); //6 wavelengths (mod C)
       if(AverageTime == 720.0 && DataCadence == 120.0) strcpy(HMISeriesLev1p,"hmi_test.S2_720s_dcon"); //8 wavelengths
       if(AverageTime == 720.0 && DataCadence == 150.0) strcpy(HMISeriesLev1p,"hmi_test.S2_720s_dcon"); //10 wavelengths
     //if(AverageTime == 360.0) strcpy(HMISeriesLev1p,"hmi_test.S_360s_dcon");
       if(AverageTime == 5760.0)strcpy(HMISeriesLev1p,"hmi_test.S_5760s_dcon");
