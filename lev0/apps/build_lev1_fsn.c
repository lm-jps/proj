...
#define H1LOGFILE "/usr/local/logs/lev1/build_lev1_fsn.%s.log"
...
ModuleArgs_t module_args[] = { 
  ...
  {ARG_INTS, "bfsn", "0", "first lev0 fsn to process. 0=error must be given by build_lev1_mgr_fsn"},
  {ARG_INTS, "efsn", "0", "last lev0 fsn to process. 0=error must be given by build_lev1_mgr_fsn"},
...
unsigned int bfsn, efsn;	//begin and end fsn to do
int verbose;
...
char argbfsn[80], argefsn[80], argquick[80];
char timetag[32];
...
int do_ingest(unsigned int bbfsn, unsigned int eefsn)
{
  ...
  sprintf(recrange, "%u-%u", bbfsn, eefsn);
...
void setup()
{
  ...
  printk("%s %s %s %s %s %s %s\n", 
	arginstru, argdsin, argdsout, argbfsn, argefsn, argquick, arglogfile);
...
int DoIt(void)
{
  ...
  printf("build_lev1_fsn done last fsn=%u\n", fsnx); //!!TEMP
  return(0);
}
