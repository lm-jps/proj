...
#define H1LOGFILE "/usr/local/logs/lev1/build_lev1_hmi.%s.log"
...
void do_quallev1(DRMS_Record_t *rs0, DRMS_Record_t *rs1, int inx, unsigned int fsn)
{
  ...
  //for instrument anomaly
  if ( (quallev0 & 0x2000) != 0) {
    quallev1 = quallev1 | Q_INSTR_ANOM1;
  }
...
int nice_intro ()
{
  ...
  if (usage)
	...
	"logfile= optional log file name. If not given uses:\n"
        "         /usr/local/logs/lev1/build_lev1_hmi.<time_stamp>.log\n");
    return(1);
    }
...
int send_mail(char *fmt, ...)
{
  ...
  va_start(args, fmt);
  vsprintf(string, fmt, args);
  sprintf(cmd, "echo \"%s\" | Mail -s \"build_lev1_hmi mail\" lev0_user", string);
  system(cmd);
  va_end(args);
  return(0);
}
...
void abortit(int stat)
{
  ...
  printk("**Exit build_lev1_hmi w/ status = %d\n", stat);
	...
	if (NEED_PATCH7(fsnx))
      ...
      if(ptinfo) {
        ...
        if(IOinfo)
        {
            ...
            drms_setkey_double(rs, "HAEX_OBS", IOdata.hae_x);
            drms_setkey_double(rs, "HAEY_OBS", IOdata.hae_y);
            drms_setkey_double(rs, "HAEZ_OBS", IOdata.hae_z);
            drms_setkey_double(rs, "GAEX_OBS", IOdata.gae_x);
            drms_setkey_double(rs, "GAEY_OBS", IOdata.gae_y);
            drms_setkey_double(rs, "GAEZ_OBS", IOdata.gae_z);
            ...
            drms_setkey_double(rs, "CRLN_OBS", IOdata.crln_obs);
            drms_setkey_double(rs, "CRLT_OBS", IOdata.crlt_obs);
            drms_setkey_double(rs, "HGLN_OBS", IOdata.hgln_obs);
            drms_setkey_double(rs, "HGLT_OBS", IOdata.crlt_obs);
...
WCSEND:
    ...
    if (fsnx >= FSN_CORRECTED_CRLN)
    {
        calver32 |= 0x1 << ((8 - 1) * 4); // 8th nibble
    }

    drms_setkey_int(rs, "CALVER32", calver32); //Put back in on 09Sep2013
...
void setup()
{
  ...
  getcwd(cwdbuf, 126);
  sprintf(idstr, "Cwd: %s\nCall: ", cwdbuf);
  sprintf(string, "build_lev1_hmi started as pid=%d ppid=%d user=%s\n",
  ...
  while(fgets(string, sizeof string, fin)) {  //get ps line
    ...
    sprintf(lfile, "%s/build_lev1_hmi_restart_%d.touch", LEV1LOG_BASEDIR, tpid);
