/*-----------------------------------------------------------------------------
 * cvs/JSOC/proj/lev0/apps/ingest_lev0_iris.c
...
int send_mail(char *fmt, ...)
{
  ...
  va_start(args, fmt);
  vsprintf(string, fmt, args);
  sprintf(cmd, "echo \"%s\" | Mail -s \"ingest_lev0_iris mail\" prodtest", string);
  system(cmd);
  va_end(args);
  return(0);
}
...
void setup()
{
  ...
  sprintf(idstr, "Cwd: %s\nCall: ", cwdbuf);
  if(grounddata)
    sprintf(string, "ingest_lev0_irisROT -g started as pid=%d user=%s\n", getpid(), username);
  else
    sprintf(string, "ingest_lev0_irisROT [-A] started as pid=%d user=%s\n", getpid(), username);
  strcat(idstr, string);
  printk("*%s", idstr);
