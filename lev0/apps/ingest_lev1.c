/*-----------------------------------------------------------------------------
 * cvs/JSOC/proj/lev1/apps/ingest_lev1.c
...
#define H1LOGFILE "/usr/local/logs/lev1/ingest_lev1.%s.log"
...
int nice_intro ()
{
  ...
  if (usage)
	...
	"logfile= optional log file name. If not given uses:\n"
        "         /usr/local/logs/lev1/ingest_lev1.<time_stamp>.log\n");
    return(1);
    }
...
void abortit(int stat)
{
  ...
  printk("**Exit ingest_lev1 w/ status = %d\n", stat);
...
void setup()
{
  ...
  getcwd(cwdbuf, 126);
  sprintf(idstr, "Cwd: %s\nCall: ", cwdbuf);
  sprintf(string, "ingest_lev1 started as pid=%d user=%s\n", getpid(), username);
  strcat(idstr, string);
  printk("%s", idstr);
  printf("%s", idstr);
  ...
  while(fgets(string, sizeof string, fin)) {  //get ps line
    ...
    sprintf(lfile, "%s/ingest_lev1_restart_%d.touch", LEV1LOG_BASEDIR, tpid);
