#ident "$Header: /home/akoufos/Development/Testing/jsoc-4-repos-0914/JSOC-mirror/JSOC/proj/lev0/apps/load_hk_config_files_iris.c,v 1.1 2013/01/24 18:39:37 jim Exp $"
/*****************************************************************************
 ...
 *
 *!!NOTE: JA 07Mar2012 Modified for simplicity for IRIS. There may be vestigial
 *        stuff left over from SDO.
 *****************************************************************************/
int load_all_apids_hk_configs(int apid, char version_number[MAX_CHAR_VERSION_NUMBER], char pkt_date[MAX_SIZE_PKT_DATE])    
{
  ...
  ptr_fvn=version_number;  //use same version # in IRIS case
...
APID_HKPFD_Files* read_all_hk_config_files(int apid, char f_version_number[MAX_CHAR_VERSION_NUMBER],char p_version_number[MAX_CHAR_VERSION_NUMBER])
{
  ...
  while( (dir_entry_p = readdir(dir_p)) != NULL ) 
  {
    ...
    **********************************/
    if( !strncmp(dir_entry_p->d_name,"IRIS-",5))
      ;//this is valid file
    else
...
//    {
      ...
      if( !strncmp(dir_entry_p->d_name,"IRIS-ISP",8) )
      {
        /* parse filenames like IRIS-ISP-version-6.89 to set pkt-version-number an apid-name  */
        sscanf(p->filename, "%*4s-%3s-%*7s-%s", p->apid_name, p->version_number); 
        p->apid=56; //set to apid value used for HK configuration data 
      }
      else if( !strncmp(dir_entry_p->d_name,"HMI-ISP",7) )
      {
    ...
    else if (!strncmp(line, "APID", 4))
    {
      ...
      else if( strstr(line, "IRIS")  )
      {
        ...
        sscanf( line,"%*s %s %d %s %s", 
                ...
                ptr_config_node->packet_id_type, 
                ptr_config_node->date);
        ptr_config_node->apid_number = 56; //special for IRIS
      }
      ...
      tmp_apid_ptr_hk_configs= tmp_apid_ptr_hk_configs->next) 
  {
		    ...
		    tmp_ptr_hk_configs->file_version_number)) 
	{
		 ...
		 tmp_ptr_gtcids_data->aia_id_version_number);
          ...
          else if(!strcmp(tmp_ptr_hk_configs->packet_id_type, IRIS_ID_TYPE))
          {
	     strcpy(tmp_ptr_hk_configs->parameter_version_number,
		 tmp_ptr_gtcids_data->hmi_id_version_number);
          }
...
int check_for_sdo_apid(int apid)
{
   //NOTE!! For IRIS always return false.
   //See Rock's mail "Re: iris hk apid" 06Mar2012 12:18
   //to check for values less than 96 decimal
   //JJA 06Mar2012
   return(0);
