...
#              where running. Script works only using LMSAL dayfile file     #
#              format. Here is example input day file: 20080415.0x01d        #
# Author:      Carl                                                          #
...
sub show_help_info
{
  ...
  print "             Example: ingest_LM_dayfile.pl apidlist=./df_apid_list_day_files dsnlist=./df_apid_ds_list src=egsefm\n\n";
  print "(1b)Ingest Day Files using apid option will ingest all files for given apid value:\n
             ingest_LM_dayfile.pl apid=<apid-value in hexidecimal format> dsnlist=<file with ds lookup list> src=<source of data>\n";
  print "             Example: ingest_LM_dayfile.pl apid=01d  dsnlist=./df_apid_ds_list src=egsefm\n\n";
  print "(1c)Ingest Day Files using date range with apidfile option:\n
             ingest_LM_dayfile.pl apidlist=<file> start=<yyyymmdd> end=< yyyymmdd> dsnlist=<file with ds lookup list> src=<source of data>\n";
  print "             Example: ingest_LM_dayfile.pl apidlist=./df_apid_list_day_files start=20070216 end=20070218 dsnlist=./df_apid_ds_list src=egsefm\n\n";
  print "(1d)Ingest Day Files using date range with apid option:\n
             ingest_LM_dayfile.pl apid=<apid in hexidecimal>  start=<yyyymmdd> end=<yyyymmdd>  dsnlist=<file with ds lookup list> src=<source of data>\n";
  print "             Example: ingest_LM_dayfile.pl apid=01d start=20070216  end=20070218 dsnlist=./df_apid_ds_list src=egsefm\n\n";
  print "(1e)Get Help Information:\n
              ingest_LM_dayfile.pl -h  or  ingest_LM_dayfile.pl -help\n\n";
  print "(1f)View what going to save in data series by adding -v as first argument. This should work for 1a,1b,1c,and 1d cases.:\n
             ingest_LM_dayfile.pl -v apidlist=<file> start=<yyyymmdd> end=< yyyymmdd> dsnlist=<file with ds lookup list> src=<source of data>\n";
  print "             Example: ingest_LM_dayfile.pl -v apidlist=./df_apid_list_day_files start=20070216 end=20070218 dsnlist=./df_apid_ds_list src=egsefm\n\n";
  print "*Note:Currently using option (1c): ingest_LM_dayfile.pl apidlist=<file containing APID list> start=<yyyymmdd> end=<yyyymmdd> dsnlist=<file with ds lookup list>  src=<Source of Data>\n";
  print "(2) Requires setup of JSD file and create series before running script.\n";
  print "(3) Requires setup of environment variable DF_LM_DAYFILE_DIRECTORY\n";
  print "(3a)Used to store location of input day files of this script.\n";
  print "(3b)Example setting: setenv DF_LM_DAYFILE_DIRECTORY /tmp20/production/hmi_hk \n";
  print "(4) Requires setup of apid list in file when use \"apidlist\" option\n";
  print "(4a)Enter values in file in hexidecimal format(i.e.,0011,001d,0015,00f).\n";
  ...
  print "(5a)Enter values in file in hexidecimal format for APID value and this data series name(i.e.,011  hmi_ground.hk_dayfile).\n";
  ...
  print "(6)****Limitation****: a)Works only on  LMSAL dayfile formats(i.e., 20071108.0x001d\n";
    ...
    foreach $file (@df_files)
    {
      ...
      if (( index  $file, $find_str ) != -1)
      {
             ...
             (( index  $file, "text" ) == -1) and ( $new ne "x" ))
        {
          ...
          if ($date_range_flg eq "1")
          {
            # check if file is within date range -this works for LMSAL day file format only
            if( (int $date_r1)  <= (int substr($file,0,8)) and
                (int $date_r2)  >= (int substr($file,0,8)) )
            {   
              push( @list, $file);
