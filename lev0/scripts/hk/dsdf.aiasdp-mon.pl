#!/usr/bin/perl
#Hacked from dsdf.aiasdp-mon.pl to get packets from aiasdp-mon instead of hmisdp-mon for rtmon flag
##############################################################################
# Name:        dsdf.aiasdp-mon.pl  - Decode and Save Day Files                          #
...
#              system.                                                       #
# Execution:   dsdf.aiasdp-mon.pl < rtmon | moc >                                       #
#              dsdf.aiasdp-mon.pl -h                                                    #
# Examples:    dsdf.aiasdp-mon.pl moc                                                   #
# Examples:    dsdf.aiasdp-mon.pl rtmon                                                 #
...
#                (8)Run dsdf.aiasdp-mon.pl with source value so that script knows which #
...
elsif ($source eq "rtmon")
{
  ...
  $pup_dir=$ENV{'DF_PICKUP_RTMON_FILES'}="/aiasdp-mon/log/packets";
...
{
  print "ERROR: unknown source value. Exting script dsdf.aiasdp-mon.pl!\n";
  print "ERROR: note hsb is not implemented in dsdf.aiasdp-mon.pl yet!\n";
  exit;
}
else 
{
...
elsif ($source eq "rtmon")
{
  ...
  #print  "dsdf.aiasdp-mon.pl:LIST HKT: @list_hkt_files\n";
  #print  "dsdf.aiasdp-mon.pl:LIST XML: @list_xml_files\n";
}
else
{
  print "ERROR: Unknown source value:<$source>. Exiting script dsdf.aiasdp-mon.pl!\n";
  exit;
}
...
# (25)close log 
print LF "--->Exiting script dsdf.aiasdp-mon.pl\n";
print LF `date -u`;
close LF;
...
print "...completed running dsdf.aiasdp-mon.pl.\n";
# End of main
...
sub do_decode_dayfile($,$,$,$,$)
{
  ...
  if($dflg == 1) {print LF "--->DEBUG:MESSAGE:dsdf.aiasdp-mon.pl:main:apid to decode::@decode_list\n";}
  ...
  foreach $decode_apid  (@decode_list)
  {
    ...
    if(substr($file,0,4) eq  $decode_apid )
    {
      ...
      if( $1 eq "ERROR" )
      {
        ...
        print LF "ERROR:Status failed when executing decode_dayfile. Exiting script dsdf.aiasdp-mon.pl.\nLOG returned:$log:\n";
...
sub do_m3sd_dayfile($,$,$,$,$)
{
  ...
  foreach $m3sd_apid  (@m3sd_list)
  {
    ...
    if((substr($file,2,2) eq  $m3sd_apid && $src eq "moc") or (substr($file,11,4) eq  $m3sd_apid && $src eq "rtmon"))
    {
      ...
      {
        # if decode_dayfile exec failed set merged=-1 for ingest_dayfile.pl
        print LF "ERROR:Status failed when executing load_m3sd. Exiting script dsdf.aiasdp-mon.pl.\nLOG returned:$log:\n";
        # set found flag to -1 meaning set merged flag to -1 because on decode list but unsuccessfully decoded dayfile
        $found_m3sd_apid = -1;
