/*
 * Module name:        nlfff_v2.c
 *
 * Description:        Wrapper module for Wiegelmann's 2012 NLFFF code (SoPh
 *
 * Calling:
 *
 ...
 * Issues:
 *          v1.0
 *          Overall structure identical to v1 except relax() defined in relax_v2.c
 *          Double checked v1 functions in rebin.c, preproc.c, green.c against original v2 code: identical
 *          Default noise mask "maskopt = 1" uses: mask = B_T / max(B_T), as suggested by Wiegemlamm+(2012)
 *          Other options defined in noisemask(), requires additional field_err segment
 *          v2.0
 *          Added wf, wd weighting, changed default to mu4=1e-3, wf=1, wd=2 as suggested by Thalmann (2019)
 *          Changed everything in relax_v2 to double, added wf, wd as parameter
 *          Preprocessing is the same as v1, except dt*=1.01 instead of 1.001, changed to use v1 instead
 *          Green's function also updated to have dz=1/sqrt(2pi)
 *          Updated rebin.c, new function rebin3d_v2()
 *          Updated output to compressed Rice
 *          No longer substitute lowest layer!
 *
 *
 * Example:
 *          nlfff_v2 "in=su_xudong.B_720s_cea_v2[12673][2017.09.06_11:36:00_TAI]" "out=su_xudong.nlfff_v2" "PREP=0" "MULTI=3" "nz=320" "nd=32" "maskopt=2"
 *
 */
...
// Noise mask
void get_mask(DRMS_Record_t *inRec,
              float *Bx, float *By, float *Bz, int nx, int ny,
              double *mask, int *maskopt_now, char *maskname);

//=============

char *module_name = "nlfff_v2";    /* Module name */

ModuleArgs_t module_args[] =
{
    {ARG_STRING, "in", NULL, "Input data series."},
    {ARG_STRING, "out", NULL,  "Output data series."},
    {ARG_INT, "maskopt", "1", "Mask functions"},
    ...
    {ARG_DOUBLE, "mu1", "1.0", "Weighting for vector magnetogram preprocessing."},
    {ARG_DOUBLE, "mu2", "1.0", "Weighting for vector magnetogram preprocessing."},
    {ARG_DOUBLE, "mu3", "0.001", "Weighting for vector magnetogram preprocessing."},
    {ARG_DOUBLE, "mu4", "0.01", "Weighting for vector magnetogram preprocessing."},
    {ARG_DOUBLE, "nue", "0.001", "Lagrangian multiplier for magnetogram ingestion."},
    ...
    {ARG_INT, "MULTI", "1", "Levels of multigrid algorithm, 0 or 1 with no multigrid."},
...
int DoIt(void)
{
    ...
    double *bx0, *by0, *bz0, *mask;
    double *bx0_now, *by0_now, *bz0_now, *mask_now;
    ...
    int nd, nd_now, maxit;
    double mu1, mu2, mu3, mu4, nue, wf, wd;
    int multi, multi_now;
    int nx_now, ny_now, nz_now, nxny_now, nynz_now, nxnynz_now;
    int maskopt, maskopt_now, runnum;
    char *comment, maskname[256];
    ...
    nue = params_get_double(&cmdparams, "nue");
    ...
    if (multi < 1) multi = 1;
    maskopt = params_get_int(&cmdparams, "maskopt");
    ...
    for (irec = 0; irec < nrecs; irec++)
    {
        ...
        mask = (double *)(calloc(nxny, sizeof(double)));
        
        // Obtain mask, in JSOC
        // If fails, falls to #0: Bh/max(Bh)
        maskopt_now = maskopt;
        inDataMask = (double *)(calloc(nxny, sizeof(double)));
        get_mask(inRec, inDataBx, inDataBy, inDataBz, nx, ny,
                 inDataMask, &maskopt_now, maskname);
        ...
        for (multi_now = multi; multi_now > 0; multi_now--) {
            ...
            mask_now = (double *)(calloc(nxny_now, sizeof(double)));
            printf("start rebin2d\n"); fflush(stdout);
            if (multi_now > 1) {
                rebin2d4(bx0, by0, bz0, mask, bx0_now, by0_now, bz0_now, mask_now, nx, ny, multi_now);
            } else {
                for (i = 0; i < nxny_now; i++) {
                    bx0_now[i] = bx0[i];
                    by0_now[i] = by0[i];
                    bz0_now[i] = bz0[i];
                }
            ...
            if (multi_now == multi) {
                ...
                rebin3d_v2(Bx_tmp, By_tmp, Bz_tmp, Bx, By, Bz, nx_now / 2, ny_now / 2, nz_now / 2);
...
//            printf("Initial guess done\n"); fflush(stdout);
            ...
            relax(bx0_now, by0_now, bz0_now, mask_now,
                  Bx, By, Bz, nx_now, ny_now, nz_now, nd_now, nue, wf, wd, maxit, verbflag);
            printf("\nRelaxation done\n"); fflush(stdout);
            /* Step 0: Grid */
            nx_now *= 2; ny_now *= 2; nz_now *= 2; nd_now *= 2;
            
            // Clean up
            free(bx0_now); free(by0_now); free(bz0_now); free(mask_now);
...
void get_mask(DRMS_Record_t *inRec,
              float *bx, float *by, float *bz, int nx, int ny,
              double *mask, int *maskopt, char *maskname)
{
    
    char *maskNames[] = {"Uniform", "Bh/Max(Bh)", "Exp"};
    if (*maskopt >= ARRLENGTH(maskNames) || *maskopt < 0) {
        *maskopt = 1;       // default
    }
    
    // Look for error segment if requested
    int status;
    ...
    if (*maskopt == 2) {
        ...
        if (status || inArray->axis[0] != nx || inArray->axis[1] != ny) {
            printf("No field_err segment, use Bh/Max(Bh) instead. \n");
            *maskopt = 1;
            drms_free_array(inArray);
        } else {
            fld_err = (double *) inArray->data;
        }
    }
    ...
    double bh_max = 0.;
    // for #2
    double fld, fld_thresh = 200.;
    double r, sig_r = 0.03;       // coefficient
    double mask0 = 0.01;
    ...
    switch (*maskopt) {
        ...
        case 1:     // Bh/max(Bh)
            // first pass
            for (int i = 0; i < nxny; i++) {
                mask[i] = sqrt(bx[i] * bx[i] + by[i] * by[i]);      // Bh
                if (bh_max < mask[i]) bh_max = mask[i];
            }
            // second pass
            for (int i = 0; i < nxny; i++) {
                mask[i] /= bh_max;
        ...
        case 2:     // Exponential
            for (int i = 0; i < nxny; i++) {
                fld = sqrt(bx[i] * bx[i] + by[i] * by[i] + bz[i] * bz[i]);
                r = fld_err[i] / fld;
                mask[i] = (fld < fld_thresh) ? (mask0) : (exp(-r / sig_r) * (1.0 - mask0) + mask0); // 0-1
            }
            drms_free_array(inArray);
            break;
    ...
    sprintf(maskname, "%s", maskNames[*maskopt]);   // Mask name
    printf("Using mask #%01d %s\n", *maskopt, maskname);
