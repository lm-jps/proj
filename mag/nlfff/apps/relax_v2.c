...
double calculateL(double *Bx, double *By, double *Bz,
                  ...
                  double *L1, double *L2,
                  double *helpL_arr, double *helpL1_arr, double *helpL2_arr);
...
void relax(double *Bxorig, double *Byorig, double *Bzorig,
           double *mask,
           double *Bx, double *By, double *Bz,
           int nx, int ny, int nz, int nd,
           double nue, double wf, double wd, int maxit, int verb)
{
    double Lx, Ly, Lz;
    double L1, L2, L3;
    ...
    double *helpL_arr = (double *) calloc(nxnynz, sizeof(double));     // Kahan sum
    double *helpL1_arr = (double *) calloc(nxnynz, sizeof(double));
    double *helpL2_arr = (double *) calloc(nxnynz, sizeof(double));
    double *helpL3_arr = (double *) calloc(nxny, sizeof(double));
...
#pragma omp parallel for private (i)
    ...
    while (it < maxit && statcount < 10 && mue > (1.0e-7 * dx * dx))
    {
        ...
        memset(helpL1_arr, 0.0, nxnynz * sizeof(double));
        memset(helpL2_arr, 0.0, nxnynz * sizeof(double));
        memset(helpL3_arr, 0.0, nxny * sizeof(double));
        L = calculateL(Bx, By, Bz,
                       ...
                       dx, dy, dz,
                       &L1, &L2,
                       helpL_arr, helpL1_arr, helpL2_arr);
        L3 = 0.0; iz = 0;     // Oct 5 2020
        for (ix = 0; ix < nx; ix++)
            for (iy = 0; iy < ny; iy++)
            {
                i = ix * nynz + iy * nz + iz;
                i1 = ix * ny + iy;
                helpL3_arr[i1] = mask[i1] * ((Bx[i] - Bxorig[i1]) * (Bx[i] - Bxorig[i1])
                                             + (By[i] - Byorig[i1]) * (By[i] - Byorig[i1])
                                             + (Bz[i] - Bzorig[i1]) * (Bz[i] - Bzorig[i1]));
            }
        kahan_sum_sgl(helpL3_arr, nxny, &L3);
        L3 = L3 / nave;
        L = L + L3;
...
#pragma omp parallel for private (i,ix,iy,iz)
            ...
            for (ix = 0; ix < nx; ix++)
                for (iy = 0; iy < ny; iy++)
                {
                    i = ix * nynz + iy * nz + iz;
                    i1 = ix * ny + iy;
                    Bx1[i] = Bx[i] - mue2 * oxby[i] - nue * mask[i1] * (Bx[i] - Bxorig[i1]);
                    By1[i] = By[i] + mue2 * oxbx[i] - nue * mask[i1] * (By[i] - Byorig[i1]);
...
#pragma omp parallel for private (i)
    ...
    free(helpL_arr);
    free(helpL1_arr); free(helpL2_arr); free(helpL3_arr);
                  ...
                  double *helpL_arr, double *helpL1_arr, double *helpL2_arr)
{
    ...
    double divB, b2, helpL, helpL1, helpL2;
...
#ifdef _OPENMP
#pragma omp parallel for private (i,ix,iy,iz,bx,by,bz,cbx,cby,cbz,fx,fy,fz,divB,b2,o2a,o2b) reduction(+:helpL) reduction(+:helpL1) reduction(+:helpL2)
#endif
    for (ix = 0; ix < nx; ix++)
        for (iy = 0; iy < ny; iy++)
            for (iz = 0; iz < nz; iz++)
            {
                ...
                if (b2 > 0.0) {
                    ...
                    helpL1_arr[i] = wa0[i] * b2 * o2a;
                    helpL2_arr[i] = wb0[i] * b2 * o2b;
                }
    ...
    kahan_sum(helpL1_arr, nxnynz, &helpL1);
    kahan_sum(helpL2_arr, nxnynz, &helpL2);
