...
//  char leer[5];
    double Bave, dt;	//, dumm1, fac1, dt0
//  double mu1, mu2, mu3, mu4, mu5;
    double dx, dy, dxdy;
    double Emag, ihelp;	//, Emag2, ihelp2
...
/*
    if(mu5 !=0.0) {
        ...
        if((streamw = fopen("halpha.dat","r")) == NULL) { printf("\n Error halpha.dat "); exit(1);}
        for(iy=0;iy<ny;iy++)
        for(ix=0;ix<nx;ix++)
        {
        i=ny*ix+iy; 
        ...
        if((streamw = fopen("w.dat","r")) == NULL) { printf("\n Error w.dat "); exit(1);}
        for(iy=0;iy<ny;iy++)
        for(ix=0;ix<nx;ix++)
        {
        i=ny*ix+iy; 
 ...
 */
    ...
    Bave = 0.0;
#ifdef _OPENMP
#pragma omp parallel for private (i,ix,iy) reduction(+:Bave)
#endif
    for (ix = 0; ix < nx; ix++)
    for (iy = 0; iy < ny; iy++) {
        i = ny * ix + iy;
...
//  printf("\n dt= %lf", dt);
    Emag = ihelp = 0.0;
#ifdef _OPENMP
#pragma omp parallel for private (i) reduction(+:Emag) reduction(+:ihelp)
#endif
    for (i = 0; i < nxny; i++) {
...
//  printf("\n L12, L3, L4 are multiplied with 1.0e6");
    while(it < maxit && dL > 0.0001 && dt > 1.0e-6)
    {
        ...
        L1 = L2 = L3 = L4 = L5 = 0.0;
#ifdef _OPENMP
#pragma omp parallel for private (i,ix,iy) reduction(+:term1a) reduction(+:term1b) reduction(+:term1c1) reduction(+:term1c2) reduction(+:term2a1) reduction(+:term2a2) reduction(+:term2b1) reduction(+:term2b2) reduction(+:term2c1) reduction(+:term2c2) reduction(+:L3)
#endif
        for (ix = 0; ix < nx; ix++)
        for (iy = 0; iy < ny; iy++)
        {
        ...
        /********** Laplace Bx,By,Bz **********/
#ifdef _OPENMP
#pragma omp parallel for private (i,ix,iy,ix1,ix2,iy1,iy2,i1,i2,i3,i4) reduction(+:L4)
#endif
        for (ix = 0; ix < nx; ix++)
        for (iy = 0; iy < ny; iy++)
        {
...
/*
        if(mu5 !=0.0)
        {
            term5=L5=0.0;
            #ifdef _OPENMP
            #pragma omp parallel for private (i,ix,iy) reduction(+:L5)
            #endif
            for(ix=0;ix<nx;ix++)
            for(iy=0;iy<ny;iy++)
            {
            ...
            L5=dxdy*L5/Emag;
            if(it%50==0) {printf(" L5= %lf",1.0e6*L5);}
        }
 */
        /********** Update B **********/
#ifdef _OPENMP
...
//  free(Bxold); free(Byold); free(Bzold);
    free(x); free(y); free(r);
    free(term3a); free(term3b); free(term3c);
