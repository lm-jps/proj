/*
 * Module name:        localpf.c
 *
 * Description:        Local Cartesian PF extrapolation using Wiegelmann's Green function code
 *
 * Calling:
 *
 ...
 *                  save ND keyword
 *              v4.0
 *                  set nd=0 by default; clean up printings
 *              v5.0
 *                  Implemented Kahan summation for all sums
 *                  modified output to save as compressed ints
 *
 * Example:
 *      localpf "in=su_xudong.B_720s_cea[11158][2011.02.15]" "out=su_xudong.localpf" "nz=256" "nd=16" "COMMENT=PF_v1_output"
 */
...
char *module_name = "localpf";    /* Module name */
...
ModuleArgs_t module_args[] =
{
    ...
    {ARG_INT, "nd", "0", "Number of grids at boundary."},
...
int DoIt(void)
{
    ...
    nz = outDims[2] = abs(params_get_int(&cmdparams, "nz"));        // z nodes fixed for dataset
    nd = abs(params_get_int(&cmdparams, "nd"));
    maxit = abs(params_get_int(&cmdparams, "maxit"));
    mu1 = fabs(params_get_double(&cmdparams, "mu1"));
    mu2 = fabs(params_get_double(&cmdparams, "mu2"));
    mu3 = fabs(params_get_double(&cmdparams, "mu3"));
    mu4 = fabs(params_get_double(&cmdparams, "mu4"));
    ...
    for (irec = 0; irec < nrecs; irec++)
    {
        ...
        for (multi_now = multi; multi_now > 0; multi_now--) {
            ...
            /* Step 3: Get Green function solution */
            if (multi_now == multi) {
