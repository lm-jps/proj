...
if mode < 0,
  ...
  if ~isempty(strfind(scenario, 'jsoc')),
    ...
    for filetype_ = JSOC_filetypes,
      ...
      fp = fopen(fn_i, filetype{4}); % sometimes 'w', sometimes 'a'
      if fp == -1,
        error('%s: could not write %s, bad template?', mfilename, fn_i);
      end;
      fprintf(fp, '# HMI %s: %s\n', filetype{2}, filetype{3});
      fprintf(fp, '# Created for run %d: %s\n', rgn.curr_run, run_name);
      fprintf(fp, '# Generated by %s on %s from matlab\n', ...
              getenv('USER'), datestr(now));
      fprintf(fp, '#\n');
      fclose(fp);
    end;
    %% append run info to the param file
    f_tag = '-param';
    fn_i = sprintf(tmpl, 'jsoc', f_tag, '', 'txt');
    fp = fopen(fn_i, 'a');
    if fp == -1,
  ...
  if ~isempty(strfind(scenario, 'jsoc')),
    %% append more to the param file
    f_tag = '-param';
    fn_i = sprintf(tmpl, 'jsoc', f_tag, '', 'txt');
    fp = fopen(fn_i, 'a');
    fprintf(fp, '# (run ended)\n');
    ...
    for filetype_ = JSOC_filetypes,
      ...
      % note, this is generically added to all the JSOC_filetypes
      fprintf(fp, '# Final frame number = %d\n', rgn.imagenum);
      fprintf(fp, '# Final frame handle = %s\n', rgn.currfile);
      fprintf(fp, '# End of run %d: %s\n', rgn.curr_run, run_name);
      fprintf(fp, '#\n');
      fclose(fp);
    end;
  end;
