...
Initialize (int maximize)
{
  ...
  MATE      = (int *) mxMalloc(allocsize);
  LINK      = (int *) mxMalloc(allocsize);
  BASE      = (int *) mxMalloc(allocsize);
  NEXTVTX   = (int *) mxMalloc(allocsize);
  LASTVTX   = (int *) mxMalloc(allocsize);
  Y         = (int *) mxMalloc(allocsize);
  NEXT_D    = (int *) mxMalloc(allocsize);
  NEXTEDGE  = (int *) mxMalloc(allocsize);

  allocsize = (U + 2 * V + 2) * sizeof(int);
  NEXTPAIR  = (int *) mxMalloc(allocsize);
#else
  allocsize = (U + 2) * sizeof(int);
