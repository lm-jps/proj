...
//    char *inQuery="hmi.lookup_ChebyCoef_TWindow", *outQuery;
    ...
    sprintf(inQueryfinal, "hmi.lookup_ChebyCoef_BNoise[%s-%s][%s]", vr_start_str, vr_stop_str, maskid_str);
    printf("%s\n", inQueryfinal);
              ...
              for (ix = 0; ix < xDim; ix++)
                {
                   ...
                   if (dToDiskCenter >= rsun || iMask >= sunSize * sunSize)
                     {
                         image[iData] = DRMS_MISSING_FLOAT;
                         continue;
                     }
                   image[iData] = (float)mask[iMask];    
                }
          }
