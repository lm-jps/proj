...
#include <jsoc_main.h>
#include "fstats.c"
#include "/homee/yliu/cvs/JSOC/proj/myproj/apps/rickmaproj/cartography.c"
#include "/homee/yliu/cvs/JSOC/proj/myproj/apps/rickmaproj/imginfo.c"
#include "/homee/yliu/cvs/JSOC/proj/myproj/apps/rickmaproj/keystuff.c"
#include "/homee/yliu/cvs/JSOC/proj/myproj/apps/rickmaproj/mdistuff.c"
...
#define INTERP_BILINEAR	(2)
						       /*  module identifier  */
char *module_name = "maprojbrfromblos";
...
ModuleArgs_t module_args[] = {
  ...
  {ARG_INT,     "blos2br", "0", "Blos converts to Br: 0 = not convert; 1 = convert"},
...
void convertblos2br(DRMS_Array_t *img, float xcen, float ycen, float rsun_obs) {
    
    int jy, yOff, iData, status;
    double yDist, yDist2, xDist, xDist2, xDist2PlusyDist2, dToDiskCenter, sinrho, cosrho;  
    int xDim = img->axis[0]; int yDim = img->axis[1], axes[2];
    float *bLos;
    bLos = (float *)img->data;
              ...
              for (ix = 0; ix < xDim; ix++)
                {
                   ...
                   sinrho = dToDiskCenter / rsun_obs;
                   cosrho = sqrt(1 - sinrho * sinrho);
                   if (cosrho > 0) bLos[iData] /= cosrho;
                   else bLos[iData] = DRMS_MISSING_FLOAT;
                }
           }
...
int DoIt (void) {
  ...
  int map_blos2br = params_get_int (params, "blos2br");
...
// Blos to Br convertion
    float xcen, ycen, rsun_obs;
        xcen = drms_getkey_float(irec, "CRPIX1", &status) - 1.0;
        ycen = drms_getkey_float(irec, "CRPIX2", &status) - 1.0;
        if (map_blos2br == 1) convertblos2br(image, xcen, ycen, img_radius);
// end of convertion
...
// image statistics
    ...
    drms_setkey_double(orec, "DATAMEDN", statMedn);
    drms_setkey_double(orec, "DATAMEAN", statMean);
    drms_setkey_double(orec, "DATARMS", statSig);
    drms_setkey_double(orec, "DATASKEW", statSkew);
    drms_setkey_double(orec, "DATAKURT", statKurt);
    drms_setkey_int(orec, "BLOS2BR", map_blos2br);
