...
// const float kNOISE_EQ = 10.0; --YL

// old modulefor MDI -- YL

const float kOutScale = 0.1;   // Not be used --YL
const int kOutBPP = 16;        // Not be used --YL
const float kNOISE_EQ = 20.0;  // Not be used so far --YL
const float sensAdjVal = 1.8;  // Not be used -- YL
...
char *module_name = "mrmlossynoptic4mdi";
...
ModuleArgs_t module_args[] =
{
   ...
   {ARG_STRING, "smallMr", "NOTSPECIFIED", ""},
   {ARG_STRING, "smallMl", "NOTSPECIFIED", ""},
   {ARG_INT, CR, "", "", ""},
   {ARG_FLOAT, "nsig", "3.0", "", ""},
...
int DoIt(void)
{
  ...
  char *cvsinfo = strdup("$Id: mrmlossynoptic4mdi.c,v 1.1 2017/05/05 00:48:47 yliu Exp $");
  ...
  mdi = cmdparams_get_int(&cmdparams, "MDI", &status);
...
/* Also, determine the minimum and maximum CT covered by each image. */
  ...
  if (modVers < 0) modVers = -modVers;
    char *inRecQuery, *outRecQuery, *outMlRecQuery, *smallMrRecQuery, *smallMlRecQuery;
    ...
    outMlRecQuery = (char *)cmdparams_get_str(&cmdparams, "outMl", &status);
    smallMrRecQuery = (char *)cmdparams_get_str(&cmdparams, "smallMr", &status);
    smallMlRecQuery = (char *)cmdparams_get_str(&cmdparams, "smallMl", &status);

    DRMS_RecordSet_t *inRD, *outRD, *outMlRD, *smallMrRD, *smallMlRD;
    DRMS_Record_t *outRec, *outMlRec, *smallMrRec, *smallMlRec;
...
// check data quality for HMI, removed -- YL

//        quality = drms_getkey_int(inRec, "QUALITY", &status);

/*
      if (mdi == 0 && quality & QUAL_CHECK)
      {
...
printf("\n######### SECOND PASS #################\n"); 
      ...
      if (!(epts = (float *) malloc(len[0] * len[1] * sizeof(float))))
        DIE("MALLOC_FAILED");
      if (!(synopbr = (float *) malloc(len[0]*len[1]*4)))
        DIE("MALLOC_FAILED");
      if (!(synopblos = (float *) malloc(len[0]*len[1]*4)))
        DIE("MALLOC_FAILED");
...
//        if (magtype == 300.0) equivPts = gMagAgg[kFiveMinuteValue];
        ...
        magtype = drms_getkey_float(inRec, "INTERVAL", &status);
        equivPts = 0.0;
...
//        equivPts = 1.0;  // with MDI 1-min and 5-min mags, update the counting -- YL
...
// compute the resized map
  ...
  float *smallSynopbr = NULL, *smallSynopblos = NULL, *smallEpts = NULL;
  int smallDims[2], xout, yout;

  xout = len[0]/nbin; yout = len[1]/(nbin-1);
  if (!(smallSynopbr = (float *) malloc(xout*yout*4))) DIE("MALLOC_FAILED");
  if (!(smallSynopblos = (float *) malloc(xout*yout*4))) DIE("MALLOC_FAILED");
  if (!(smallEpts = (float *) malloc(xout*yout*4))) DIE("MALLOC_FAILED");
  smallDims[0] = xout; smallDims[1] = yout;
  frebinbox(synopbr, smallSynopbr, len[0], len[1], nbin, nbin-1);
...
// regular-size synoptic chart
  sinBbase = 0.5 - len[1]/2.0;
  sinBdelta = 2.0/len[1];

  for (jrow = 0; jrow < len[1]; jrow++) {
    rowoffset = jrow * len[0];
    sinB = sinBdelta * (sinBbase + jrow);
    sinB2 = sinB * sinB;
    cosB = sqrt(1.0 - sinB2);

    for (icol = 0; icol < len[0]; icol++) {
        if (isnan(synopbr[rowoffset + icol]))
           {
             synopblos[rowoffset + icol] = DRMS_MISSING_FLOAT;
             continue;
           }
        synopblos[rowoffset + icol] = synopbr[rowoffset + icol] * cosB;
    }
  }

// small-size synoptic chart

  sinBbase = 0.5 - yout/2.0;
  sinBdelta = 2.0/yout;

  for (jrow = 0; jrow < yout; jrow++) {
    rowoffset = jrow * xout;
    sinB = sinBdelta * (sinBbase + jrow);
    sinB2 = sinB * sinB;
    cosB = sqrt(1.0 - sinB2);

    for (icol = 0; icol < xout; icol++) {
        if (isnan(smallSynopbr[rowoffset + icol]))
           {
             smallSynopblos[rowoffset + icol] = DRMS_MISSING_FLOAT;
             continue;
           }
        smallSynopblos[rowoffset + icol] = smallSynopbr[rowoffset + icol] * cosB;
    }
  }

/* end of Blos calculation */

    outRD = drms_create_records(drms_env, 1, outRecQuery, DRMS_PERMANENT, &status);
    if (status)
        DIE("Output record not created");
    outMlRD = drms_create_records(drms_env, 1, outMlRecQuery, DRMS_PERMANENT, &status);
    if (status)
        DIE("Output record not created");
    smallMrRD = drms_create_records(drms_env, 1, smallMrRecQuery, DRMS_PERMANENT, &status);
    if (status)
        DIE("Output record not created");
    ...
    if (status)
        ...
        DRMS_Segment_t *outSeg, *outMlSeg, *smallMrSeg, *smallMlSeg;
        DRMS_Array_t *outArray, *smallArray;
        int outDims[2] = {len[0],len[1]};
        outRec = outRD->records[0];
        outMlRec = outMlRD->records[0];
        smallMrRec = smallMrRD->records[0];
        smallMlRec = smallMlRD->records[0];

	// write Mr synop
	outSeg = drms_segment_lookupnum(outRec, 0);
	outArray = drms_array_create(DRMS_TYPE_FLOAT, 2, outDims, synopbr, &status);
	status = drms_segment_write(outSeg, outArray, 0);
        if (status) DIE("problem writing file");

  if (fstats(len[0]*len[1], synopbr, &statMin, &statMax, &statMedn, &statMean, &statSig,
      &statSkew, &statKurt, &statNgood)) printf("\n Statistics computation failed\n");

        drms_setkey_int(outRec, "TOTVALS", len[0]*len[1]);
        drms_setkey_int(outRec, "DATAVALS", statNgood);
        drms_setkey_int(outRec, "MISSVALS", len[0]*len[1]-statNgood);
        ...
        // write Ml synop
        outMlSeg = drms_segment_lookupnum(outMlRec, 0);
        outArray = drms_array_create(DRMS_TYPE_FLOAT, 2, outDims, synopblos, &status);
        status = drms_segment_write(outMlSeg, outArray, 0);
        if (status) DIE("problem writing file");
//        set_statistics(outMlSeg, outArray, 1);

  if (fstats(len[0]*len[1], synopblos, &statMin, &statMax, &statMedn, &statMean, &statSig,
      &statSkew, &statKurt, &statNgood)) printf("\n Statistics computation failed\n");

        drms_setkey_int(outMlRec, "TOTVALS", len[0]*len[1]);
        drms_setkey_int(outMlRec, "DATAVALS", statNgood);
        drms_setkey_int(outMlRec, "MISSVALS", len[0]*len[1]-statNgood);
        ...
        // write epts for Mr and Ml synop
        outSeg = drms_segment_lookupnum(outRec, 1);
        outMlSeg = drms_segment_lookupnum(outMlRec, 1);
        ...
        // write Mr smallSynop
        smallMrSeg = drms_segment_lookupnum(smallMrRec, 0);
        smallArray = drms_array_create(DRMS_TYPE_FLOAT, 2, smallDims, smallSynopbr, &status);
        status = drms_segment_write(smallMrSeg, smallArray, 0);
        if (status) DIE("problem writing file");

  if (fstats(xout*yout, smallSynopbr, &smallstatMin, &smallstatMax, &smallstatMedn, &smallstatMean, &smallstatSig,
      &smallstatSkew, &smallstatKurt, &smallstatNgood)) printf("\n Statistics computation failed\n");

        drms_setkey_int(smallMrRec, "TOTVALS", xout*yout);
        drms_setkey_int(smallMrRec, "DATAVALS", smallstatNgood);
        drms_setkey_int(smallMrRec, "MISSVALS", xout*yout-smallstatNgood);
        drms_setkey_double(smallMrRec, "DATAMIN", smallstatMin);
        drms_setkey_double(smallMrRec, "DATAMAX", smallstatMax);
        drms_setkey_double(smallMrRec, "DATAMEDN", smallstatMedn);
        drms_setkey_double(smallMrRec, "DATAMEAN", smallstatMean);
        drms_setkey_double(smallMrRec, "DATARMS", smallstatSig);
        drms_setkey_double(smallMrRec, "DATASKEW", smallstatSkew);
        drms_setkey_double(smallMrRec, "DATAKURT", smallstatKurt);
        drms_free_array(smallArray);

        // write Ml smallSynop
        smallMlSeg = drms_segment_lookupnum(smallMlRec, 0);
        smallArray = drms_array_create(DRMS_TYPE_FLOAT, 2, smallDims, smallSynopblos, &status);
        status = drms_segment_write(smallMlSeg, smallArray, 0);
        if (status) DIE("problem writing file");

  if (fstats(xout*yout, smallSynopblos, &smallstatMin, &smallstatMax, &smallstatMedn, &smallstatMean, &smallstatSig,
      &smallstatSkew, &smallstatKurt, &smallstatNgood)) printf("\n Statistics computation failed\n");

        drms_setkey_int(smallMlRec, "TOTVALS", xout*yout);
        drms_setkey_int(smallMlRec, "DATAVALS", smallstatNgood);
        drms_setkey_int(smallMlRec, "MISSVALS", xout*yout-smallstatNgood);
        drms_setkey_double(smallMlRec, "DATAMIN", smallstatMin);
        drms_setkey_double(smallMlRec, "DATAMAX", smallstatMax);
        drms_setkey_double(smallMlRec, "DATAMEDN", smallstatMedn);
        drms_setkey_double(smallMlRec, "DATAMEAN", smallstatMean);
        drms_setkey_double(smallMlRec, "DATARMS", smallstatSig);
        drms_setkey_double(smallMlRec, "DATASKEW", smallstatSkew);
        drms_setkey_double(smallMlRec, "DATAKURT", smallstatKurt);
        drms_free_array(smallArray);

        // write smallEpts for Mr smallSynop
        smallMrSeg = drms_segment_lookupnum(smallMrRec, 1);
        smallMlSeg = drms_segment_lookupnum(smallMlRec, 1);
...
//for reserved keywords
        ...
        drms_setkey_string(outMlRec, "DATE", tstr);
...
//for image coordinate mapping keywords
        ...
        drms_setkey_string(outMlRec, "CTYPE1", "CRLN-CEA");
        drms_setkey_string(outRec, "CTYPE2", "CRLT-CEA");
        drms_setkey_string(outMlRec, "CTYPE2", "CRLT-CEA");
...
//        i=len[0]/2+0.5;
        ...
        drms_setkey_double(outMlRec, "CRPIX1", i);
        ...
        drms_setkey_double(outMlRec, "CRPIX2", l);
...
//        carrtime = (cr-1)*360.0 + 180.0 - (double)(0.5 * 360.0/len[0]);
        ...
        drms_setkey_double(outMlRec, "CRVAL1", carrtime);
        i=0.0;
        drms_setkey_double(outRec, "CRVAL2", i);
        drms_setkey_double(outMlRec, "CRVAL2", i);
        l=-360.0/len[0];
        ...
        drms_setkey_string(outMlRec, "CUNIT2", "Sine Latitude");
...
//        drms_setkey_string(outRec, "T_REC", tstr);
        drms_setkey_float(outRec, "CADENCE", 360.0);
        drms_setkey_float(outMlRec, "CADENCE", 360.0);
        drms_setkey_float(outRec, "T_REC_step", 27.2753*24.*60.*60.);
        drms_setkey_float(outMlRec, "T_REC_step", 27.2753*24.*60.*60.);
// image statistics
...
printf("keywords wring half-way done\n");
  if (fstats(len[0]*len[1], synopblos, &statMin, &statMax, &statMedn, &statMean, &statSig,
      &statSkew, &statKurt, &statNgood)) printf("\n Statistics computation failed\n");
...
//Synoptic map keywords
        ...
        drms_setkey_string(outMlRec, "T_OBS", tstr);
        drms_setkey_string(outMlRec, "T_ROT", tstr);
        sprint_at(tstr, tstart - delta_T);
        drms_setkey_string(outRec, "T_START", tstr);
        drms_setkey_string(outMlRec, "T_START", tstr);
        sprint_at(tstr, tstop - delta_T);
        drms_setkey_string(outRec, "T_STOP", tstr);
        drms_setkey_string(outMlRec, "T_STOP", tstr);
        ...
        if (carrStretch > 0)
          {
            ...
            drms_setkey_int(outMlRec, kCARSTRCH, carrStretch);
            drms_setkey_float(outMlRec, kDIFROT_A, diffrotA);
            drms_setkey_float(outMlRec, kDIFROT_B, diffrotB);
            drms_setkey_float(outMlRec, kDIFROT_C, diffrotC);
          }
...
//for reserved keywords
        ...
        sscanf(tstr, "%d.%d.%d", &y, &m, &d);
        sprintf(tstr, "%04d-%02d-%02d", y, m, d);
        drms_setkey_string(smallMrRec, "DATE", tstr);
        drms_setkey_string(smallMlRec, "DATE", tstr);
//        drms_setkey_string(smallRec, "BUNIT", "Mx/cm^2");

//for image coordinate mapping keywords
        
        drms_setkey_string(smallMrRec, "HISTORY", historyofthemodule);
        drms_setkey_string(smallMrRec, "CTYPE1", "CRLN-CEA");
        drms_setkey_string(smallMrRec, "CTYPE2", "CRLT-CEA");
        drms_setkey_string(smallMrRec, "BLD_VERS", jsoc_version);
        status = drms_setkey_string(smallMrRec, "CODEVER", cvsinfo);
        drms_setkey_string(smallMlRec, "HISTORY", historyofthemodule);
        drms_setkey_string(smallMlRec, "CTYPE1", "CRLN-CEA");
        drms_setkey_string(smallMlRec, "CTYPE2", "CRLT-CEA");
        drms_setkey_string(smallMlRec, "BLD_VERS", jsoc_version);
        status = drms_setkey_string(smallMlRec, "CODEVER", cvsinfo);

//        i=xout/2+0.5;
        double tmp;
        tmp=xout - ((180.0-((nbin+1)/2.0-1.0)*(360.0/len[0]))/(360.0/xout)+1.0) + 1.0;
           // The first pixel coordinate is ((nbin+1)/2.0-1.0)*(360.0/len[0]) because of the boxcar average.
        ...
        drms_setkey_double(smallMrRec, "CRPIX1", tmp);
        drms_setkey_double(smallMlRec, "CRPIX1", tmp);
        tmp=(yout+1.0)/2;
        drms_setkey_double(smallMrRec, "CRPIX2", tmp);
        drms_setkey_double(smallMlRec, "CRPIX2", tmp);
//        carrtime = (cr-1)*360.0 + 180.0 - (double)(0.5 * 360.0/xout);
        carrtime = (cr-1)*360.0 + 180.0;
        drms_setkey_double(smallMrRec, "CRVAL1", carrtime);
        drms_setkey_double(smallMlRec, "CRVAL1", carrtime);
        tmp=0.0;
        drms_setkey_double(smallMrRec, "CRVAL2", tmp);
...
//        drms_setkey_string(smallRec, "T_REC", tstr);
        drms_setkey_float(smallMrRec, "CADENCE", 360.0);
        drms_setkey_float(smallMlRec, "CADENCE", 360.0);
        drms_setkey_float(smallMrRec, "T_REC_step", 27.2753*24.*60.*60.);
        drms_setkey_float(smallMlRec, "T_REC_step", 27.2753*24.*60.*60.);
// image statistics
// compute statistics
/*
  if (fstats(xout*yout, smallSynopbr, &smallstatMin, &smallstatMax, &smallstatMedn, &smallstatMean, &smallstatSig,
      &smallstatSkew, &smallstatKurt, &smallstatNgood)) printf("\n Statistics computation failed\n");
        ...
        drms_setkey_double(smallRec, "BRMIN", smallstatMin);
        drms_setkey_double(smallRec, "BRMAX", smallstatMax);
        drms_setkey_double(smallRec, "BRMEDN", smallstatMedn);
        drms_setkey_double(smallRec, "BRMEAN", smallstatMean);
        drms_setkey_double(smallRec, "BRRMS", smallstatSig);
        drms_setkey_double(smallRec, "BRSKEW", smallstatSkew);
        drms_setkey_double(smallRec, "BRKURT", smallstatKurt);

  if (fstats(xout*yout, smallSynopblos, &smallstatMin, &smallstatMax, &smallstatMedn, &smallstatMean, &smallstatSig,
      &smallstatSkew, &smallstatKurt, &smallstatNgood)) printf("\n Statistics computation failed\n");

        drms_setkey_double(smallRec, "BLOSMIN", smallstatMin);
        drms_setkey_double(smallRec, "BLOSMAX", smallstatMax);
        drms_setkey_double(smallRec, "BLOSMEDN", smallstatMedn);
        drms_setkey_double(smallRec, "BLOSMEAN", smallstatMean);
        drms_setkey_double(smallRec, "BLOSRMS", smallstatSig);
        drms_setkey_double(smallRec, "BLOSSKEW", smallstatSkew);
        drms_setkey_double(smallRec, "BLOSKURT", smallstatKurt);
*/
//Synoptic map keywords

        drms_setkey_string(smallMrRec, "T_OBS", tstr);
        drms_setkey_string(smallMrRec, "T_ROT", tstr);
        drms_setkey_string(smallMlRec, "T_OBS", tstr);
        drms_setkey_string(smallMlRec, "T_ROT", tstr);
        sprint_at(tstr, tstart - delta_T);
        drms_setkey_string(smallMrRec, "T_START", tstr);
        drms_setkey_string(smallMlRec, "T_START", tstr);
        sprint_at(tstr, tstop - delta_T);
        drms_setkey_string(smallMrRec, "T_STOP", tstr);
        drms_setkey_string(smallMlRec, "T_STOP", tstr);
//        sprint_at(tstr, tearth);
//        drms_setkey_string(smallRec, "T_EARTH", tstr);
        drms_setkey_int(smallMrRec, "CAR_ROT", cr);
        drms_setkey_double(smallMrRec, "CARRTIME", carrtime);
        drms_setkey_int(smallMlRec, "CAR_ROT", cr);
        drms_setkey_double(smallMlRec, "CARRTIME", carrtime);
        solephem(trot, eph);
//        HeliographicLocation(trot, &i, &l, &b);
        drms_setkey_double(smallMrRec, "B0_ROT", eph[9]/RADSINDEG);
        drms_setkey_double(smallMlRec, "B0_ROT", eph[9]/RADSINDEG);
//        HeliographicLocation(tstart, &i, &l, &b);
        solephem(tstart, eph);
        drms_setkey_double(smallMrRec, "B0_FRST", eph[9]/RADSINDEG);
        drms_setkey_double(smallMlRec, "B0_FRST", eph[9]/RADSINDEG);
//        HeliographicLocation(tstop, &i, &l, &b);
        solephem(tstop, eph);
        drms_setkey_double(smallMrRec, "B0_LAST", eph[9]/RADSINDEG);
        drms_setkey_double(smallMlRec, "B0_LAST", eph[9]/RADSINDEG);
//        drms_setkey_double(smallRec, "EARTH_B0", bearth);
        l=(cr-1)*360.0+((nbin+1)/2.0-1.0)*(360.0/len[0]);
        drms_setkey_double(smallMrRec, "LON_FRST", l);
        drms_setkey_double(smallMlRec, "LON_FRST", l);
        l=cr*360.0-360.0/len[0]-((nbin+1)/2.0-1.0)*(360.0/len[0]);
        drms_setkey_double(smallMrRec, "LON_LAST", l);
        drms_setkey_double(smallMlRec, "LON_LAST", l);
        l=-360.0/xout;
        drms_setkey_double(smallMrRec, "LON_STEP", l);
        drms_setkey_double(smallMlRec, "LON_STEP", l);
        l=center;
        drms_setkey_double(smallMrRec, "W_OFFSET", l);
        drms_setkey_string(smallMrRec, "W_WEIGHT", "Even");
        drms_setkey_double(smallMlRec, "W_OFFSET", l);
        drms_setkey_string(smallMlRec, "W_WEIGHT", "Even");
        i=lastidx-firstidx+1;
        drms_setkey_int(smallMrRec, "IMG_NUM", i);
        drms_setkey_int(smallMlRec, "IMG_NUM", i);
        sprint_at(tstr, imrec[firstidx].tobs);
        drms_setkey_string(smallMrRec, "IMG_FRST", tstr);
        drms_setkey_string(smallMlRec, "IMG_FRST", tstr);
        sprint_at(tstr, imrec[lastidx].tobs);
        drms_setkey_string(smallMrRec, "IMG_LAST", tstr);
        drms_setkey_string(smallMlRec, "IMG_LAST", tstr);
        sprint_at(tstr, magtime);
        drms_setkey_string(smallMrRec, "IMG_ROT", tstr);
...
// save the differential rotation correction parameters 
        if (carrStretch > 0)
          {
            drms_setkey_int(smallMrRec, kCARSTRCH, carrStretch);
            drms_setkey_float(smallMrRec, kDIFROT_A, diffrotA);
            drms_setkey_float(smallMrRec, kDIFROT_B, diffrotB);
            drms_setkey_float(smallMrRec, kDIFROT_C, diffrotC);
            drms_setkey_int(smallMlRec, kCARSTRCH, carrStretch);
            drms_setkey_float(smallMlRec, kDIFROT_A, diffrotA);
            drms_setkey_float(smallMlRec, kDIFROT_B, diffrotB);
            drms_setkey_float(smallMlRec, kDIFROT_C, diffrotC);
          }
  ...
  drms_close_records(smallMrRD, DRMS_INSERT_RECORD);
...
*/
	 ...
	 float sum;
	 int npts;
         float sumfinal = 0.0, eptscount = 0.0;
         int nptsfinal = 0;
...
//	       {
		  ...
		  dev = fabs(val[j] - med);
//                  if (nptsfinal >= nEquivPtsReq) break; // for general purpose
                  if (eptscount >= nEquivPtsReq) break; // for MDI only --YL
                  if (dev < nsig * sig)
                  {
                     sumfinal += val[j];
                     eptscount += ept[j]; // for MDI only -- YL 
                     nptsfinal += 1;                 
//                   nptsfinal += ept[j];
                     //--npts;  --YL
                     //++nrej;  --YL
...
//	 if (npts)
         if (nptsfinal)
	 {
	    ...
	    if (synVal < minVal)
	    {
	       synopbr[row * len[0] + col] = minVal;
	    }
	    else if (synVal > maxVal)
	    {
	       synopbr[row * len[0] + col] = maxVal;
	    }
	    else
	    {
	       synopbr[row * len[0] + col] = synVal;
	    }
	 }
	 else
	 {
	    synopbr[row * len[0] + col] = DRMS_MISSING_FLOAT;
	 }
	  
	 ww[row * len[0] + col] = npts;
	 epts[row * len[0] + col] = eptscount; // for MDI because 1-min and 5-min mags have different epts --YL
...
/*
$Id: mrmlossynoptic4mdi.c,v 1.1 2017/05/05 00:48:47 yliu Exp $
$Source: /home/akoufos/Development/Testing/jsoc-4-repos-0914/JSOC-mirror/JSOC/proj/mag/synop/apps/mrmlossynoptic4mdi.c,v $
$Author: yliu $
*/
 ...
 *            
 * $Log: mrmlossynoptic4mdi.c,v $
 * Revision 1.1  2017/05/05 00:48:47  yliu
 * New module to produce MDI synoptic charts
 *
