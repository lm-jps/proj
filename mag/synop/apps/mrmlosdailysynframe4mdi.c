...
char *module_name = "mrmlosdailysynframe4mdi";
...
int DoIt(void)
{
  DRMS_RecordSet_t *inRSfinal, *outRD, *outMlRD, *smallRD, *smallMlRD;
  ...
  TIME t_rec, t_rec0;
  char *t_window = "1440"; //hard-coded T-window; 1-day for MDI
  ...
  unsigned int qualmask1, qualmask2, qualmask3; // for MDI only --YL
  int i, j, crn;
  ...
  char *sdate, *stime, *inQuery, *outQuery, *outMlQuery, *smallMlRecQuery, *synQuery;
  char *drmethod;
...
// end of definition
  ...
  char *cvsinfo = strdup("$Id: mrmlosdailysynframe4mdi.c,v 1.1 2017/05/06 00:18:06 yliu Exp $");
  ...
  inRSfinal = drms_open_records(drms_env, inQuery, &status);
...
// quality check for MDI -- YL
        ...
        if ((quality != qualmask1) & (quality != qualmask2) & (quality != qualmask3))
           {
