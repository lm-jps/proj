...
typedef struct MagCol_struct
{
  ...
  float *datacolBr; /* Ptr to column of data elonging to one magnetogram */
  float *datacolBt; /* Ptr to column of data elonging to one magnetogram */
  float *datacolBp; /* Ptr to column of data elonging to one magnetogram */
...
int magStats(float **valBr, int npts, double sum, int outThreshold, double *avg, float *med);
...
int DoIt(void)
{
  ...
  float *synopBr, *synopBt, *synopBp;
  int *wt;
  char *ww;
  float *epts = NULL;
//  float *losSynop = NULL;
  float *psBr, *psBt, *psBp, *pmBr, *pmBt, *pmBp;
  char *odir;
  struct stat stbuf;
  float nsig;
  ...
  char *cvsinfo = strdup("$Id: hmib3compsynoptic4cr2136.c,v 1.1 2016/06/24 22:59:37 arta Exp $");
...
printf("\n######### SECOND PASS #################\n"); 
      ...
      if (!(synopBr = (float *) malloc(len[0]*len[1]*4)))
        DIE("MALLOC_FAILED");
      if (!(synopBt = (float *) malloc(len[0]*len[1]*4)))
        DIE("MALLOC_FAILED");
      if (!(synopBp = (float *) malloc(len[0]*len[1]*4)))
        DIE("MALLOC_FAILED");
    ...
    for (idx = 0; idx < ngood; idx++) 
      {
        ...
        DRMS_Array_t *inArrayBr, *inArrayBt, *inArrayBp;
...
//        inArray = drms_segment_read(inSeg, DRMS_TYPE_FLOAT, &status);
        ...
        inSeg = drms_segment_lookup(inRec, "Br");
        inArrayBr = drms_segment_read(inSeg, DRMS_TYPE_FLOAT, &status);

        if (status)
            {
            printf(" No data file found, status=%d\n", status);
            drms_free_array(inArrayBr);
            continue;
            }

        inSeg = drms_segment_lookup(inRec, "Bt");
        inArrayBt = drms_segment_read(inSeg, DRMS_TYPE_FLOAT, &status);

        if (status)
            {
            printf(" No data file found, status=%d\n", status);
            drms_free_array(inArrayBr);
            drms_free_array(inArrayBt);
            continue;
            }

        inSeg = drms_segment_lookup(inRec, "Bp");
        inArrayBp = drms_segment_read(inSeg, DRMS_TYPE_FLOAT, &status);
	    ...
	    mMagCol.datacolBr = (float *)malloc(sizeof(float) * len[1]);
            mMagCol.datacolBt = (float *)malloc(sizeof(float) * len[1]);
            mMagCol.datacolBp = (float *)malloc(sizeof(float) * len[1]);
	    mMagCol.equivPts = equivPts;
	    mMagCol.ds = imrec[idx].ds;
	    mMagCol.col = col;
	    mMagCol.valid = 1;
	   
	    pmBr = (float *)inArrayBr->data + col;
            pmBt = (float *)inArrayBt->data + col;
            pmBp = (float *)inArrayBp->data + col;

	    psBr = mMagCol.datacolBr;
            psBt = mMagCol.datacolBt;
            psBp = mMagCol.datacolBp;
...
// compute the resized map
  ...
  float *smallSynopBr = NULL, *smallSynopBt = NULL, *smallSynopBp = NULL, *smallEpts = NULL;
  int smallDims[2], xout, yout;

  xout = len[0]/nbin; yout = len[1]/(nbin-1);
  if (!(smallSynopBr = (float *) malloc(xout*yout*4))) DIE("MALLOC_FAILED");
  if (!(smallSynopBt = (float *) malloc(xout*yout*4))) DIE("MALLOC_FAILED");
  if (!(smallSynopBp = (float *) malloc(xout*yout*4))) DIE("MALLOC_FAILED");
...
//  frebin(epts, smallEpts, len[0], len[1], nbin);
  ...
  frebinbox(synopBt, smallSynopBt, len[0], len[1], nbin, nbin-1);
  frebinbox(synopBp, smallSynopBp, len[0], len[1], nbin, nbin-1);
...
// compute statistics
  ...
  if (fstats(len[0]*len[1], synopBr, &statMin, &statMax, &statMedn, &statMean, &statSig, 
      &statSkew, &statKurt, &statNgood)) printf("\n Statistics computation failed\n");
        ...
        drms_setkey_int(outRec, "BRVALS", statNgood);
        i = len[0]*len[1]-statNgood;
        drms_setkey_int(outRec, "BRMISSVA", i);
  ...
  if (fstats(len[0]*len[1], synopBt, &statMin, &statMax, &statMedn, &statMean, &statSig,
      &statSkew, &statKurt, &statNgood)) printf("\n Statistics computation failed\n");

        drms_setkey_int(outRec, "BTVALS", statNgood);
        i = len[0]*len[1]-statNgood;
        drms_setkey_int(outRec, "BTMISSVA", i);
        drms_setkey_double(outRec, "BTMIN", statMin);
        drms_setkey_double(outRec, "BTMAX", statMax);
        drms_setkey_double(outRec, "BTMEDN", statMedn);
        drms_setkey_double(outRec, "BTMEAN", statMean);
        drms_setkey_double(outRec, "BTRMS", statSig);
        drms_setkey_double(outRec, "BTSKEW", statSkew);
        drms_setkey_double(outRec, "BTKURT", statKurt);

  if (fstats(len[0]*len[1], synopBp, &statMin, &statMax, &statMedn, &statMean, &statSig,
      &statSkew, &statKurt, &statNgood)) printf("\n Statistics computation failed\n");

        drms_setkey_int(outRec, "BPVALS", statNgood);
        i = len[0]*len[1]-statNgood;
        drms_setkey_int(outRec, "BPMISSVA", i);
        drms_setkey_double(outRec, "BPMIN", statMin);
        drms_setkey_double(outRec, "BPMAX", statMax);
        drms_setkey_double(outRec, "BPMEDN", statMedn);
        drms_setkey_double(outRec, "BPMEAN", statMean);
        drms_setkey_double(outRec, "BPRMS", statSig);
        drms_setkey_double(outRec, "BPSKEW", statSkew);
        drms_setkey_double(outRec, "BPKURT", statKurt);
  ...
  if (fstats(xout*yout, smallSynopBr, &smallstatMin, &smallstatMax, &smallstatMedn, &smallstatMean, &smallstatSig,
      &smallstatSkew, &smallstatKurt, &smallstatNgood)) printf("\n Statistics computation failed\n");
        ...
        drms_setkey_int(smallRec, "BRVALS", smallstatNgood);
        i = xout*yout-smallstatNgood;
        drms_setkey_int(smallRec, "BRMISSVA", i);
  ...
  if (fstats(xout*yout, smallSynopBt, &smallstatMin, &smallstatMax, &smallstatMedn, &smallstatMean, &smallstatSig,
      &smallstatSkew, &smallstatKurt, &smallstatNgood)) printf("\n Statistics computation failed\n");

        drms_setkey_int(smallRec, "BTVALS", smallstatNgood);
        i = xout*yout-smallstatNgood;
        drms_setkey_int(smallRec, "BTMISSVA", i);
        drms_setkey_double(smallRec, "BTMIN", smallstatMin);
        drms_setkey_double(smallRec, "BTMAX", smallstatMax);
        drms_setkey_double(smallRec, "BTMEDN", smallstatMedn);
        drms_setkey_double(smallRec, "BTMEAN", smallstatMean);
        drms_setkey_double(smallRec, "BTRMS", smallstatSig);
        drms_setkey_double(smallRec, "BTSKEW", smallstatSkew);
        drms_setkey_double(smallRec, "BTKURT", smallstatKurt);

  if (fstats(xout*yout, smallSynopBp, &smallstatMin, &smallstatMax, &smallstatMedn, &smallstatMean, &smallstatSig,
      &smallstatSkew, &smallstatKurt, &smallstatNgood)) printf("\n Statistics computation failed\n");

        drms_setkey_int(smallRec, "BPVALS", smallstatNgood);
        i = xout*yout-smallstatNgood;
        drms_setkey_int(smallRec, "BPMISSVA", i);
        drms_setkey_double(smallRec, "BPMIN", smallstatMin);
        drms_setkey_double(smallRec, "BPMAX", smallstatMax);
        drms_setkey_double(smallRec, "BPMEDN", smallstatMedn);
        drms_setkey_double(smallRec, "BPMEAN", smallstatMean);
        drms_setkey_double(smallRec, "BPRMS", smallstatSig);
        drms_setkey_double(smallRec, "BPSKEW", smallstatSkew);
        drms_setkey_double(smallRec, "BPKURT", smallstatKurt);
...
*/
	...
	status = drms_segment_write(outSeg, outArray, 0);
        ...
        // synop (second segment) -- Bt
        outSeg = drms_segment_lookup(outRec, "Bt");
        outArray = drms_array_create(DRMS_TYPE_FLOAT, 2, outDims, synopBt, &status);
        status = drms_segment_write(outSeg, outArray, 0);
        if (status)
          DIE("problem writing file");
        drms_free_array(outArray);

        // synop (third segment) -- Bp
        outSeg = drms_segment_lookup(outRec, "Bp");
        outArray = drms_array_create(DRMS_TYPE_FLOAT, 2, outDims, synopBp, &status);
        status = drms_segment_write(outSeg, outArray, 0);
        if (status)
          DIE("problem writing file");
        drms_free_array(outArray);
	...
	status = drms_segment_write(outSeg, outArray, 0);
        ...
        // smallSynop (second segment) -- Bt
        smallSeg = drms_segment_lookupnum(smallRec, 1);
        smallArray = drms_array_create(DRMS_TYPE_FLOAT, 2, smallDims, smallSynopBt, &status);
        status = drms_segment_write(smallSeg, smallArray, 0);
        if (status)
          DIE("problem writing file");
        drms_free_array(smallArray);

        // smallSynop (third segment) -- Bp
        smallSeg = drms_segment_lookupnum(smallRec, 2);
        smallArray = drms_array_create(DRMS_TYPE_FLOAT, 2, smallDims, smallSynopBp, &status);
        status = drms_segment_write(smallSeg, smallArray, 0);
        if (status)
          DIE("problem writing file");
        drms_free_array(smallArray);

        // smallEpts (fourth segment)
        smallSeg = drms_segment_lookupnum(smallRec, 3);
...
//   cosCenter = cos(center * M_PI / 180);
   ...
   {
      float *valBr = (float *)malloc(sizeof(float) * wt[col]);
      float *valBt = (float *)malloc(sizeof(float) * wt[col]);
      float *valBp = (float *)malloc(sizeof(float) * wt[col]);
	 ...
	 memset(pcols, 0, sizeof(MagCol_t) * wt[col]);
      ...
      if (!valBr || !ept || !valBt || !valBp) printf("MALLOC FAILED"); 
...
*/
	 ...
	 float sum;
	 int npts;
         float sumfinalBr = 0.0, sumfinalBt = 0.0, sumfinalBp = 0.0;
         int nptsfinal = 0;
...
//	 if (radialFound) cosrho = cosLat * cosCenter;
	 ...
	 {
	    float magValBr = *(smc[col][i].datacolBr + row);
            float magValBt = *(smc[col][i].datacolBt + row);
            float magValBp = *(smc[col][i].datacolBp + row);
	    if (!drms_ismissing_float(magValBr)) 
	    {
	       nEquivPts += smc[col][i].equivPts;
	       sum += magValBr;
	       valBr[j] = magValBr;
...
//	    if (radialFound) noiseLevel = noiseLevel * MIN(1 / cosrho, maxNoiseAdj);
	    ...
	    memcpy(statVals, valBr, sizeof(float) * npts);
	    nStatPts = magStats(&statVals, npts, sum, minOutPts, &avg, &med);
	    /* Reject outliers whose values exceeds the noise threshold */
            // this rejection is only based on Br component. For Bt and Bp, contribution
            // of the pixels are the same as Br.
...
//	 if (npts)
         if (nptsfinal)
	 {
	    ...
	    float synValBr = sumfinalBr/nptsfinal;
            float synValBt = sumfinalBt/nptsfinal;
            float synValBp = sumfinalBp/nptsfinal;
	    ...
	    else if (synValBr > maxVal)
	    {
	       synopBr[row * len[0] + col] = maxVal;
	    }
	    else
	    {
	       synopBr[row * len[0] + col] = synValBr;
	    }

            if (synValBt < minVal)
            {
                synopBt[row * len[0] + col] = minVal;
            }
            else if (synValBt > maxVal)
            {
               synopBt[row * len[0] + col] = maxVal;
            }
            else
            {
               synopBt[row * len[0] + col] = synValBt;
            }

            if (synValBp < minVal)
            {
                synopBp[row * len[0] + col] = minVal;
            }
            else if (synValBp > maxVal)
            {
               synopBp[row * len[0] + col] = maxVal;
            }
            else
            {
               synopBp[row * len[0] + col] = synValBp;
            }
	 ...
	 else
	 {
	    synopBr[row * len[0] + col] = DRMS_MISSING_FLOAT;
            synopBt[row * len[0] + col] = DRMS_MISSING_FLOAT;
            synopBp[row * len[0] + col] = DRMS_MISSING_FLOAT;
	 }
...
void FreeMagColsData(int start, int end, int incr, int *n, MagCol_t **mc)
{
   if (mc)
   {
      float *dataBr = NULL, *dataBt = NULL, *dataBp = NULL;
      int cIdx;
      int idx;
	 ...
	 {
	    dataBr = mc[cIdx][idx].datacolBr;
            dataBt = mc[cIdx][idx].datacolBt;
            dataBp = mc[cIdx][idx].datacolBp;
	    if (dataBp)
	    {
	       free(dataBp);
	       mc[cIdx][idx].datacolBp = NULL;
	    }
            if (dataBt)
            {
               free(dataBt);
               mc[cIdx][idx].datacolBt = NULL;
            }
            if (dataBr)
            {
               free(dataBr);
               mc[cIdx][idx].datacolBr = NULL;
               mc[cIdx][idx].valid = 0;
            }
	 }
      }
...
/*
$Id: hmib3compsynoptic4cr2136.c,v 1.1 2016/06/24 22:59:37 arta Exp $
$Source: /home/akoufos/Development/Testing/jsoc-4-repos-0914/JSOC-mirror/JSOC/proj/mag/synop/apps/hmib3compsynoptic4cr2136.c,v $
$Author: arta $
*/
 ...
 *            
 * $Log: hmib3compsynoptic4cr2136.c,v $
 * Revision 1.1  2016/06/24 22:59:37  arta
 * not sure what this is bu thte build needs it
 *
