 ...
 * disambig "in=su_xudong.test_patch_vec[2010.03.29_12:00:00_TAI][2]" "out=su_xudong.test_patch_vec_a" "geometry=1"
 */
...
/* cmdparams defined in jsoc_main.h:  extern CmdParams_t cmdparams; */
    ...
    for (irec = 0; irec < nrecs; irec++) {
   	...
   	radius = asin(rsun_ref / dsun_obs) * Rad2arcsec / cdelt;
        ...
        harpnum = drms_getkey_int(inRec, "HARPNUM", &status);
        if (status)
            printf("status false\n");
        else
            printf("HARP number found:%d",harpnum);
        ...
        if (geometry == 1) {
            ...
            nx = maskArray->axis[0]; ny = maskArray->axis[1];
printf("nx=%d, ny=%d\n", nx, ny); fflush(stdout);
                DIE("Mask dimensions checked");
