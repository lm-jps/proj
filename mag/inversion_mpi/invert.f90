SUBROUTINE INVERT (OBS_LONG,SCAT_LONG,GUESS,RES,ERR)
  !
  ! J M Borrero
  ! Dec 16, 2009
  ...
  REAL(DP), INTENT(OUT), DIMENSION(11)                :: ERR
  !---------------------------------------------------------------------
...
! By RCE: Change "IF (MAXVAL(OBS(:,1)).GT.1E-2) THEN"  by " .GT. 1E4 

IF (MAXVAL(OBS(:,1)).GT. TREIC) THEN
     ...
     !By RCE: Call initial guess routine
     IF (TOTPOL .GT. 1d-3) THEN 
     CALL WFA_GUESS(OBS/MAXVAL(OBS(:,1)),LANDA0,GUESS(6),GUESS(2),GUESS(3),GUESS(7), TOTPOL)
     ENDIF

     BESTMODEL(:)=GUESS
     MODEL(:,1)=GUESS

     !By RCE: For initialization of the source function:
     MODEL(8,1) = 0.4 * OBS(1,1)*0.98
     MODEL(9,1) = 0.6 * OBS(1,1)*0.98
     ...
     !---------------------------------------
!  By RCE: ENDIF used to be here.. now it's a few lines down
!write(*,*) 'Maxval Obs',MAXVAL(OBS(:,1))
!write(*,*)  'obs Long' ,OBS_LONG
!write(*,*)   'Nbins', NBINS
!write(*,*) 'Minval',MINVAL(OBS(:,1))
  !---------------
  ! Getting Errors
  !---------------
  RES=BESTMODEL
