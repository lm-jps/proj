...
/* Map of pixels where input data are Nan */
    for (n = 0; n < imgpix; n++)
    {
      for (m = 0; m < nvar; m++)
      {
        if (isnan (data[n + m*imgpix])){for (m = 0; m < nvar; m++) data[n + m*imgpix] = 0.0; map_nan[n] = 1;}
    } }
    printf("data is read\n");
  } // end if mpi_rank == 0
...
/* then send the partials to the other PEs */
  ...
  MPI_Barrier(MPI_COMM_WORLD);
  if (mpi_rank == 0) printf("data had propagated to all PE.\n");
...
/* now do inversion in parallel */
  ...
  {
#if KEIJISKIP
   if ((n % rows > rows * 18/32) &&
       (n % rows < rows * 19/32) &&
       (n > istart + (iend - istart) * 0 / 16) &&
       (n < istart + (iend - istart) * 8 / 16))
   {
#endif
   ...
   {
    for (j=0; j<paramct; j++){FinalResLocal[(n-istart)*paramct+j]=1.0;} // put dummy
    for (k=0; k<Err_ct ; k++){FinalErrLocal[(n-istart)*Err_ct +k]=0.0;}
   }
#endif
  }
  printf("Hello, this is %d th PE : done inversion\n", mpi_rank);
  MPI_Barrier(MPI_COMM_WORLD);
