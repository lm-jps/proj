/* store_ds_fd_V_01h_lev1_8.c
 * 	stores mdi fd_V_01h_lev1_8 ds in DRMS/SUMS.
*/
/* Usage:  store_ds_fd_V_01h_lev1_8 series_num=n-m
 *         
 ...
 * to the su_ds.fd_V_01h_lev1_8.jsd where:
 *		Index:          series_num
 *
 */
...
char *module_name = "store_ds_fd_V_01h_lev1_8";
char *series = "ds_mdi.fd_V_01h_lev1_8"; 
...
int nice_intro()
  ...
  if (usage)
    {
    printf("store_ds_fd_V_01h_lev1_8 {-h} {-v} series_num=n-m\n"
...
for(i = s_start; i <= s_end; i++) {
  sprintf(cmd,"peq 'prog:mdi,level:lev1.8,series:fd_V_01h[%d]' 1> %s 2>&1", 
		i, logname);
  ...
  sprintf(fname, "%s/fd_V_01h.%d.overview.fits", wd, i);
...
if ((status = drms_setkey_string(rec, "T_BEGIN", t_begin)))
   ...
   if (status == DRMS_ERROR_UNKNOWNKEYWORD) {
     sprintf(msgtxt, 
	     "ERROR: series %s does not have a keyword named 'T_BEGIN'\n", series);
     Mailit(msgtxt);
   }
   else {
