...
*
       ...
       dm     = idatetime(4) ! diff between local time and UT. For example, PDT = UT - 420 min.
       rhh    = idatetime(5) + idatetime(6) / 60.0 ! local time in float-hour.
     &                       - float(dm)    / 60.0 ! then make it in UT
       ...
       rdd = float(iday) + rhh / 24.0 - 1.0 ! MIND 1 day must be offset when used in date2lng().

       call date2lng(cr,rlon,yy1,mm1,rdd)

*       write(*,*) cr, yy1, mm1, rdd, rhh
...
* very approx. B0
       ...
       write(*,'(f6.1,1x,f6.1)') 90.0 - b0, rlon
