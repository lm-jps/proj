...
*
       ...
       itm(4) = idatetime(4) ! diff to UT in minutes
...
*       rdd = float(iday) - 1.0 + rhh/24.0
       rdd = float(iday) - 1.0 + 0.5 ! 12UT
       call date2lng(cr,rlon,yy1,mm1,rdd)
*       write(*,'('' Now at CR'',i5,'' Lon'',f7.2)') cr, rlon
       write(*,*) cr
       write(*,*) int(rlon)
