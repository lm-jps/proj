...
C        character*100 filsolx,filsoly
	integer*4 retval
...
c       inversion related arrays
        ...
        dimension  ov1(nmd,nmd),vv1x(nmd,nx0), cov(2,nmd,nmd)
...
C 49      format('#target rad, err-suppr. param, CG of av. kernel, ')
C 50      format('# 1st,2nd,3rd quart. pt, (3rd-1st)quart.pt., soln, err')
...
C        open(10,file=filsolx)
C        open(20,file=filsoly)
...
C      if (itnum.eq.1 .and. j.le.1) then
C      print*, ctil1x(i,jj), fakc(j,i), sumccx
C      endif
1114    continue
        avcx(j)=sumccx
...
421	continue
	...
	errx(itnum) = er1x
	erry(itnum) = er1y
C        write(10,54)x0(itnum), amu, cencx(j), (dthcx(i),i=3,1,-1),
C     1  s1x, sum1x, er1x
C        write(20,54)x0(itnum), amu, cency(j), (dthcy(i),i=3,1,-1),
C     1  s1y, sum1y, er1y
