...
#define CARTOGRAPHY_VERSION_NUM     (1.1)
 ...
 *  09.07.02	copied to private location for inclusion in DRMS modules and
 *	standalone code; removed optimum_scale(), pow2scale(), name2proj(),
 *	proj2name()
 *  09.12.02	fixed two icc11 compiler warnings
 *  14.02.06	fixed over-limb sinusoidal equal-area mapping bug for
 *	plane2sphere (not thoroughly tested)
 *  14.08.01	restored name2proj(), proj2name() from original source
 *	(but removing "Aerial")
 *  18.05.04	trapped roundoff problem for arg of asin in img2sphere
 *	(but it might still occur in other functions!)
