#define SOHO_EPHEM_VERSION_NUM     (1.1)
...
#ifdef SOHO_EPHEM_TABLE
  strcpy (summary_file, SOHO_EPHEM_TABLE);
#else
  fprintf (stderr, "Error (soho_ephemeris): unknown location for SOHO table\n");
  return -1;
#endif
  if (firstcall) {
 ...
 */
  ...
  if (soho_ephemeris (secapp, &au, &lat, &lon, &vr, &vn, &vw, &modt) < 0)
    return 0.0 / 0.0;
