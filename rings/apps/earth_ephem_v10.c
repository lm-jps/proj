...
#define TABLEDIR		("/home/rick/src/ephem/tables")
 ...
 */
  ...
  if (jd < 2378448.5) {
	...
	tstr);
    return 1;
  } else if (jd < 2433264.5) sprintf (tablename, "%s/unxp1800.406", TABLEDIR);
  else if (jd < 2451536.5) sprintf (tablename, "%s/unxp1950.405", TABLEDIR);
  else if (jd < 2469808.5) sprintf (tablename, "%s/unxp2000.405", TABLEDIR);
  else {
    sprint_time (tstr, gcitdt, "Z", -1);
