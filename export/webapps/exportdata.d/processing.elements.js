...
function processingGetRecInfo(exportOption)
{
    var time_pkey = null;
    var first_record = null;
    var keyword_list = null;
    var segment_list = null;
    ...
    time_pkey = ($('ExportRecordSet').retrieve('first_time_pkey', null) !== null ? $('ExportRecordSet').retrieve('first_time_pkey', null) : 'T_REC');
    keyword_list = [ time_pkey, time_pkey + '_step', 'CAR_ROT', 'CRLN_OBS', 'CRLT_OBS', 'CDELT1', 'CTYPE1' ];
    segment_list = $('ExportRecordSet').retrieve('first_fits_segment', null);
    series = $('ExportRecordSet').retrieve('series', null);
    first_record_recnum = $('ExportRecordSet').retrieve('first_record_recnum', null);
    last_record_recnum = $('ExportRecordSet').retrieve('last_record_recnum', null);
    ...
    if (series !== null && first_record_recnum != null && last_record_recnum != null)
    {
        ...
        {
            $('ProcessingWaitMessage').innerHTML = error_message;
        };
        ...
        options =
        {
            ...
            onSuccess: function(response)
            {
                ...
                if (gri_response_obj !== null)
                {
                    ...
                    {
                        error_message = 'processing failure obtaining record-set information: ' + JSON.stringify(gri_response_obj.error_message);
                    }
                    else
                    {
            ...
            {
                var error_message = 'processing failure getting record-set information: ' + JSON.stringify(response.responseJSON.error_message);

                return on_failure(error_message);
            },
            on428: function(response)
            {
                var error_message = 'processing invalid arguments to record-set endpoint: ' + JSON.stringify(response.responseJSON.error_message);
...
function HmiB2ptrCheck(fromSetProcessing)
{
    ...
    if (series.toUpperCase() === 'hmi.B_720s'.toUpperCase())
    {
...
function AiaScaleCheck(fromSetProcessing)
{
    ...
    // 'aia_scale' processing); otherwise, data go into series + '_mod' (indicated by 'aia_scale_mod' processing)

    if ($('ExportRecordSet').retrieve('is_aia_lev1', false))
    {
        ...
        outputSeries = series.strip().replace(/lev1/i, 'lev1p5');
        ...
        if (outputSeries == AIA_LEV1P5)
        {
            ...
            $("ExportFilenameFmt").value = compatibleFormat.replace(AIA_LEV1P5, series.strip()).replace(/[.]{requestid}/i, '');
        }
        else
        {
    ...
    if (!$('AiaScalePerformCutoutCheckbox').checked)
    {
        ...
        if (0 && series.strip().toLowerCase().search(/aia[.]lev1/) == 0)
        {
...
function ResizeCheck(fromSetProcessing)
  ...
  if ($("ResizeSunCenter").checked)
    ...
    if ($("ResizeDoScale").checked)
    {
        ...
        {
            // first record
            ResizeSizeRatio = fl_record_info.keywords[keyCDELT].values[0] / parseFloat($("ResizeCdelt").value);
        }
    }
  else
    rv = rv + ",rescale=0";
...
function ImPatchSet(param)
  {
    ...
    // record count cannot be null - you have to have a valid record-set before you can click on im_patch processing.
    if ($('ExportRecordSet').retrieve('count', null) === null)
    {
        $('ExportRecordSet').store({ "fl_record_info" : null });        
...
function ImPatchCheck(fromSetProcessing)
  ...
  if ($("ImTStart").value == "NotSpecified")
    {
    if (fl_record_info !== null)
      $("ImTStart").value = fl_record_info.keywords[keyTIME].values[0];
    }
  if ($("ImTStart").value == "NotSpecified")
    {
  ...
  if ($("ImTStop").value == "NotSpecified")
    {
    if (fl_record_info !== null)
      $("ImTStop").value = fl_record_info.keywords[keyTIME].values[1];
    }
  if ($("ImTStop").value == "NotSpecified")
    {
  ...
  if ($("ImTDelta").value == "NotSpecified" && fl_record_info !== null)
    {
    ...
    if (posAt > 0)
      ...
      $("ImTDelta").value = fl_record_info.keywords[keySTEP].values[0] + "s";
    }
  ...
  if ($("ImLocType").selectedIndex == 3)
    {
    if ($("ImCarrot").value == "NotSpecified" && fl_record_info !== null)
      $("ImCarrot").value = fl_record_info.keywords[keyCARROT].values[0];
  ...
  if ($("ImLocType").selectedIndex == 3)
    {
    if ($("ImX").value == "NotSpecified" && fl_record_info !== null)
      $("ImX").value = fl_record_info.keywords[keyCRLN].values[0];
    }
...
// XXXXXXX need to set sizeratio based on high and wide, can use keyCDELT if needed
  ...
  if (fl_record_info !== null)
    {
    var dims = fl_record_info.segments[0].dims[0];
...
function MaprojCheck(fromSetProcessing)
{
  ...
  if (fl_record_info !== null)
    ...
    var DegPerPixel = new Number(fl_record_info.keywords[keyCDELT].values[0] * DegPerArcsec);
  ...
  if ($("MaprojLnObs").checked) $("MaprojX").value = fl_record_info.keywords[keyCRLN].values[0];
