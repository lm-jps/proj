...
#define out_namespace "sha.dsds"

extern int start_pvm(int (*msg)(const char *, ...));
extern int prod_host_set(void);
    ...
    int sig;
{
  ...
  if(nocontrolc) {
    ...
    if(ccactive) {
      ...
      if((list = call_dsds(&list,REQTAPECANCEL,dsds_tid,pe_tid,(int(*)(char *, ...))printf,debugflg)) == NULL) {
...
KEY *form_arg_data_out(PSERVER *sptr, argument *arg)
{
  ...
  char cmd[MAX_STR], newmdirec[MAX_STR], buf[128];
...
*/
  ...
  if(reqmegs) {
    if(firstalloc) {
      ...
      rs = drms_create_record (drms_env, out_namespace, DRMS_PERMANENT, &dstatus);
/*    rs = drms_create_record (drms_env, drmsname, DRMS_PERMANENT, &dstatus);  */
      if(dstatus) {
        printk("**ERROR %d: Can't create record in %s\n", dstatus, out_namespace);
        abortit(1);
      }
  ...
  /* save this info in jpelist for others in this archive group to check  */
	...
	jix++;
        ...
        rs = (DRMS_Record_t *)getkey_fileptr(JPElist, ext); // Ugh - using a DRMS_Record_t * to hold a FILE *.
        sprintf(ext, "%s_%d_rs", arg->key, i);
        setkey_fileptr(&sptr->map_list, ext, (FILE *)rs);
      }
    }
  ...
  while(arg->kind != ARG_END) {
    if(arg->kind == ARG_DATA_IN || arg->kind == ARG_DATA_OUT) {
      ...
      for(i=0; i < nsets; i++) {
        ...
        sprintf(rdbline, "%s\t%s\t%d\t%s\t%d\t%s\t%d\t%lu\t%g",
...
void call_archive(PSERVER *stab, HDATA *hdata, KEY *rlist, int status)
{
  ...
  while(arg->kind != ARG_END) {         	/* find all output datasets */
    if(arg->kind == ARG_DATA_OUT) {
      ...
      drms_setkey_string (rsx, "prog", getkey_str (alist, "prog"));
      drms_setkey_string (rsx, "level", getkey_str (alist, "level"));
      drms_setkey_string (rsx, "series", getkey_str (alist, "series"));
  ...
  if (!stab->archive || stab->archive == 't') { /* don't archive if 0 or temp
    ...
    /* if permanent ds, give it about 27 years to expire  */
    if (stab->archive == 'p') rsx->seriesinfo->retention = 10000;
  }
  ...
  while(arg->kind != ARG_END) {		/* find all output datasets */
    if(arg->kind == ARG_DATA_OUT) {
      ...
      for(i=0; i < outnsets; i++) {	/* do for each dataset */
        ...
        wd = getkey_str(alist, "wd");
        dsize = getkey_double(alist, "dsds_bytes");
        if((blist = (KEY *)call_dsds(&alist, REQUPDATE0, dsds_tid, pe_tid, (int(*)(char *, ...))printf, debugflg)) == NULL) {
