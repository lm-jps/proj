/*-----------------------------------------------------------------------------
 * /home/production/cvs/JSOC/proj/jpe/apps/jsnxqry.c
 *-----------------------------------------------------------------------------
 *
 *  jsnxqry.c to qry the database given prog, level, series and start series_num
 *  and end series_num.
 *
 * Just like jsnqry but also shows bytes, create_date and archive_tape.
 *
 * Usage: jsnxqry -pprog -llevel -sseries -bstart_sn -eend_sn
...
printf("here we are in DoIt()\n"); //!!TEMP
  ...
  while(1) {
    for(next_sn = start_sn; next_sn <= end_sn; next_sn++) {
      ...
      printk("prog:%s,level:%s,series:%s[%d] %0.13g %s %s\n", 
	a_prog, a_level, a_series, next_sn, sinfo->bytes, sinfo->creat_date, sinfo->arch_tape);
