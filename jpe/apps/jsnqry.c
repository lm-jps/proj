/*-----------------------------------------------------------------------------
 * /home/production/cvs/JSOC/proj/jpe/apps/jsnqry.c
 *-----------------------------------------------------------------------------
 *
 *  jsnqry.c to qry the database given prog, level, series and start series_num
 *  and end series_num.
 *
 * Usage: jsnqry -pprog -llevel -sseries -bstart_sn -eend_sn
...
char a_prog[32], a_level[32], a_series[32];
char line[1024];
char *username;
char *in_ds;
KEY *dslist;
...
void get_args(int argc, char *argv[])
{
	int c;
	if(argc != 6) {
		printf("Usage: jsnqry -pprog -llevel -sseries -bstart_sn -eend_sn\n");
...
int DoIt()
{
  ...
  while(1) {
    for(next_sn = start_sn; next_sn <= end_sn; next_sn++) {
      ...
      //drms_sprint_rec_query(rstring, rset->records[0]); 
      //printk("%s\n", rstring);
 
      printk("prog:%s,level:%s,series:%s[%d]\n", 
		a_prog, a_level, a_series, next_sn);
      drms_close_record(rset->records[0], DRMS_FREE_RECORD);
    }
    if(wildflg) {
