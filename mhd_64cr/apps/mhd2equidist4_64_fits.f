...
* number of grid for each three direction.
        ...
        parameter(iout = 80 - 1, jout = 36 - 1, kout = 72 - 1)
        real*8  roo(0:iout,0:jout,0:kout),tto(0:iout,0:jout,0:kout)
...
*        write(*,*) ' Input Ncr Nstep (negative = quit)'
*        read(*,*) ncr, nstep
        ncr = 1000
        nstep = 4000
        if ((ncr .LT. 0) .OR. (nstep .LT. 0)) goto 998

        call readmhd(ncr,nstep,lmhd)
        if (.NOT. (lmhd)) goto 999
        write(*,*) ' read MHD data ... done :', ncr, nstep
 ...
 111    continue
        ...
        if (lfits) then
          write(*,*) ' now making FITS file'
          call outfits()
        else
          write(*,*) ' now making plain text file'
          call outtext()
        endif
        ncr2 = ncr
        nstep2 = nstep
...
* number of grid for each three direction.
        ...
        integer ivar
*
        integer istatus, iunit, blocksize
        integer bitpix, igroup
        integer naxis, naxis1, naxis2, naxis3
        integer naxes(3)
        logical lsimple, lextend
        integer i, j, k
*
        integer iout, jout, kout
        parameter(iout = 80 - 1, jout = 36 - 1, kout = 72 - 1)
        real    ftsarray(kout+1,jout+1,(iout+1)*8)
*        real*8  ftsarray(kout+1,jout+1,(iout+1)*8)
*
        ...
        do j = 1, jout + 1
        do k = 1, kout + 1 
          ftsarray(k,j,           i) = roo(i-1,jout-j+1,k-1)
          ftsarray(k,j,(iout+1)*1+i) = tto(i-1,jout-j+1,k-1)
          ftsarray(k,j,(iout+1)*2+i) = uro(i-1,jout-j+1,k-1)
          ftsarray(k,j,(iout+1)*3+i) = uto(i-1,jout-j+1,k-1)
          ftsarray(k,j,(iout+1)*4+i) = upo(i-1,jout-j+1,k-1)
          ftsarray(k,j,(iout+1)*5+i) = bro(i-1,jout-j+1,k-1)
          ftsarray(k,j,(iout+1)*6+i) = bto(i-1,jout-j+1,k-1)
          ftsarray(k,j,(iout+1)*7+i) = bpo(i-1,jout-j+1,k-1)
        enddo
        enddo
        enddo

        blocksize=1 ! always 1 ?
        istatus=0 ! initialize FITS I/O and get LUN anyway
        call ftgiou(iunit,istatus)

* delete file if exist
        filename = 'd3equidist.fits'
        call ftopen (iunit,filename,1,blocksize,istatus) ! 1 = readonly
        if (istatus .EQ. 0) then ! if exist
          call ftdelt(iunit,istatus)
        else if (istatus .EQ. 103) then ! if "clearly" non-exist
          istatus=0
          call ftcmsg
        else ! otherwise ... sometimes occur...delete anyway
          istatus=0
          call ftcmsg
          call ftdelt(iunit,istatus)
        endif

        call ftfiou(iunit,istatus) ! release LUN etc.
        call ftgiou(iunit,istatus) ! get an "unused" LUN

        call ftinit(iunit,filename,blocksize,istatus) ! open empty file to write-out

* header variables
*        bitpix = -64  ! if desiring the output be of double-precision float ! note this is not necessary same as var()
        bitpix = -32 ! if desiring the output be of single-precision float
        naxis = 3     ! this case, 3D
        naxes(3)=(iout+1)*8! num of grids.
        naxes(2)= jout+1
        naxes(1)= kout+1
        lsimple = .true.
        lextend = .true.
        call ftphpr(iunit,lsimple,bitpix,naxis,naxes,
     &              0,1,lextend,istatus) ! write minimum required header
        naxis1 = naxes(1)
        naxis2 = naxes(2)
        naxis3 = naxes(3)
* write-out values
        igroup = 1
*        call ftp3dd(iunit,igroup,naxis1,naxis2,naxis1,naxis2,naxis3, ! if ftsarray() is of double precision
*     &              ftsarray,istatus)
        call ftp3de(iunit,igroup,naxis1,naxis2,naxis1,naxis2,naxis3, ! if ftsarray() is of single precision
     &              ftsarray,istatus)

* add keyword to header ..... must be before fpd3de/fpd3dd ?
        call ftpkys(iunit,'VARIABLE',
     &              'N,T,V_rtp,B_rtp',' ',istatus)
        call ftpkyj(iunit,'NumLayer',(iout+1),
     &             'num. of layer for each variable',
     &             istatus)

        call ftpkys(iunit,'dRfix','0.05Rs',' ',istatus)  ! MIND these three must be edited
        call ftpkys(iunit,'dtheta','5 deg.',' ',istatus)
        call ftpkys(iunit,'dphi','5 deg.',' ',istatus)

        call ftpkys(iunit,'UNIT1','1/cc ',' ',istatus)
        call ftpkys(iunit,'UNIT2','MK   ',' ',istatus)
        call ftpkys(iunit,'UNIT3','km/s ',' ',istatus)
        call ftpkys(iunit,'UNIT4','gauss',' ',istatus)

        call ftclos(iunit, istatus) ! Close FITS file
        call ftfiou(iunit, istatus) ! Free Log. Unit. #
