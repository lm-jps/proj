...
* number of grid for each three direction.
        ...
        parameter(iout = 80 - 1, jout = 72 - 1, kout = 144 - 1)
        ...
        integer nx, ny, nz
        parameter(nx=kout+1, ny=jout+1, nz=(iout+1)*8)
        real*8  indata(nx,ny,nz)
...
!          var8(8) = var8(8) * b0 * 1.0D+05 ! Bphi     = -By
          var8(6) = var8(6) * b0 ! BR       =  Bx in gauss
          var8(7) = var8(7) * b0 ! Btheta   = -Bz
          var8(8) = var8(8) * b0 ! Bphi     = -By
 ...
 111    continue
        ...
        do j = 1, jout + 1
        do k = 1, kout + 1 
          indata(k,j,           i) = roo(i-1,jout-j+1,k-1) ! adjust historical mismatches..
          indata(k,j,(iout+1)*1+i) = tto(i-1,jout-j+1,k-1)
          indata(k,j,(iout+1)*2+i) = uro(i-1,jout-j+1,k-1)
          indata(k,j,(iout+1)*3+i) = uto(i-1,jout-j+1,k-1)
          indata(k,j,(iout+1)*4+i) = upo(i-1,jout-j+1,k-1)
          indata(k,j,(iout+1)*5+i) = bro(i-1,jout-j+1,k-1)
          indata(k,j,(iout+1)*6+i) = bto(i-1,jout-j+1,k-1)
          indata(k,j,(iout+1)*7+i) = bpo(i-1,jout-j+1,k-1)
        enddo
        enddo
        enddo
        open(unit=11,file='d3equidist144x72.txt',status='unknown')
        do k = 1, nz
        do j = 1, ny
        do i = 1, nx
          write(11,'(e13.6)') indata(i,j,k)
        enddo
        enddo
        enddo
        close(11)
...
*
        ...
        open(unit=2,file='d3equidist144x72.dat',status='unknown')
        do k = 0, kout
        do j = 0, jout
        do i = 0, iout
