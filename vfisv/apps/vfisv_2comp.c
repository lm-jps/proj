 ...
 *
 *   33) 2022 Feb 10 --  ABGM version to invert pixels with filling factor different than 1
 *        (a) Add new keywords:
 *               "Filling Factor Inversion (default:0)": 0 means inversion with one atmospheric component
 *                                                       1 means inversion with two atmospheric components
 *               "xcen coordinate (default:2047)": x coordinate of the center of the fov to be inverted
 *               "ycen coordinate (default:2047)": y coordiante of the center of the fov to be inverted
 *               "x radius (default:2048)": value of the radius of the fov to be inverted in x direction 
 *               "y radius (default:2048)": value of the radius of the fov to be inverted in y direction
 *               "ff value (default:1.0)": value of filling factor in case you do not want to be fixed to 1
 *               "lower polarization degree (default:0.0025)": lower value of polarization degree of pixels 
 *                                                             to be inverted
 *               "upper polarization degree (default:100.0)": upper value of polarization degree of pixels 
 *                                                            to be inverted
 *
 * ------------------------------------------------------------------------------------------------------ */
...
#if HARPATCH == 1
char *module_name = "vfisvcombine FD10 HARP"; // may be kept unchanged
#else
char *module_name = "vfisvcombine FD10";      // may be kept unchanged
#endif
...
extern void invert_ (double *, double *, double *, double *, double *, double[*][*], int *, double *, double *, double *); // after Feb 10, 2011
...
//ABGM
  {ARG_INT,     "invert_ff",                   "0", "1: ff is inverted; 0: ff is fixed to 1 (default:1)"},
  {ARG_INT,     "xcen",                     "2047", "x center coordinate of the fov to invert (default:2047)"},
  {ARG_INT,     "ycen",                     "2047", "y center coordinate of the fov to invert (default:2047)"},
  {ARG_INT,     "xrad",                     "2048", "x radius of the fov to invert (default:2048)"},
  {ARG_INT,     "yrad",                     "2048", "y radius of the fov to invert (default:2048)"},
  {ARG_DOUBLE,  "ff_value",                  "1.0", "Value of Filling Factor (default:1.0)"},
  {ARG_DOUBLE,  "pol_low",                "0.0025", "Lower polarization degree (default:0.0025)"},
  {ARG_DOUBLE,  "pol_upp",                 "100.0", "Upper polarization degree (default:100.0)"},
//ABGM
#if NLEVPIXL == 1
...
//ABGM
  const int    INVERT_FFc              = params_get_double(params, "invert_ff");
  const int    XCENc                   = params_get_double(params, "xcen");
  const int    YCENc                   = params_get_double(params, "ycen");
  const int    XRADc                   = params_get_double(params, "xrad");
  const int    YRADc                   = params_get_double(params, "yrad");
  const double FF_VALUEc               = params_get_double(params, "ff_value");
  const double POL_LOWc                = params_get_double(params, "pol_low");
  const double POL_UPPc                = params_get_double(params, "pol_upp");
//ABGM
#if NLEVPIXL == 1
...
/* important variables for inversions */
  ...
  //ABGM
  if (guess[9] != FF_VALUE) {guess[9] = FF_VALUE;}  
  //ABGM
  int keyfreep;
...
// -- will add identification here for mod-L mode -- YL
/*
        cameraID = drms_getkey_int(inRec,"CAMERA",&status);
        if (cameraID == 3)
           {
...
// -- these lines are moved here from previous lines -- YL
/*
#if NLEVPIXL == 1
  NOISE_LEVELFI   = NOISE_LEVELFIc;
  NOISE_LEVELFQ   = NOISE_LEVELFQc;
...
/* Map of invalid values (NaN or all-zero) , or off-disk, out of rectangle of interest, or out of the HARP region etc. */
    for (n = 0; n < imgpix; n++)
    {
      ...
      // fypix = ((float)(iy) -(crpixy - 1.0)) * cdelty;
      fxpix = ((float)(ix) -(crpixx - 1.0 - (crpixx - (XCEN+1))));  //ABGM 
      fypix = ((float)(iy) -(crpixy - 1.0 - (crpixy - (YCEN+1))));  //ABGM 
      if (abs(fxpix) > XRAD) {nan_map[n] = 2;}  // turn on flag to-be-skipped for being out-of-disk   //ABGM 
      if (abs(fypix) > YRAD) {nan_map[n] = 2;}  // turn on flag to-be-skipped for being out-of-disk   //ABGM 

      fxpix = fxpix * cdeltx;  //ABGM 
      fypix = fypix * cdelty;  //ABGM 
...
/* ME inversion initialization */
  ...
  if (INVERT_FF == 1){list_free_params[9]=1;}
 ...
 * All filter are normalized here. */
      ...
      //ABGM
      invert_ (obs, scat, guess, res, err, filters, &iconverge_flag, weights, &POL_LOW, &POL_UPP); // added polarization degree on Feb 10, 2022
      //ABGM
...
char *meinversion_version(){return strdup("$Id: vfisv_2comp.c,v 1.1 2022/03/04 17:31:39 arta Exp $");}
